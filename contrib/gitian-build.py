...
# Copyright (c) 2020-2021 The QRAX developers
...
def build():
    ...
    os.makedirs('qrax-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../qrax/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])

    if args.linux:
        print('\nCompiling ' + args.version + ' Linux')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'qrax='+args.commit, '--url', 'qrax='+args.url, '../qrax/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../gitian.sigs/', '../qrax/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call('mv build/out/qrax-*.tar.gz build/out/src/qrax-*.tar.gz ../qrax-binaries/'+args.version, shell=True)

    if args.windows:
        print('\nCompiling ' + args.version + ' Windows')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'qrax='+args.commit, '--url', 'qrax='+args.url, '../qrax/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-unsigned', '--destination', '../gitian.sigs/', '../qrax/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call('mv build/out/qrax-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/qrax-*.zip build/out/qrax-*.exe build/out/src/qrax-*.tar.gz ../qrax-binaries/'+args.version, shell=True)

    if args.macos:
        print('\nCompiling ' + args.version + ' MacOS')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'qrax='+args.commit, '--url', 'qrax='+args.url, '../qrax/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-unsigned', '--destination', '../gitian.sigs/', '../qrax/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call('mv build/out/qrax-*-osx-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/qrax-*.tar.gz build/out/qrax-*.dmg build/out/src/qrax-*.tar.gz ../qrax-binaries/'+args.version, shell=True)
...
def sign():
    ...
    subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs/', '../qrax/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    subprocess.check_call('mv build/out/qrax-osx-signed.dmg ../qrax-binaries/'+args.version+'/qrax-'+args.version+'-osx.dmg', shell=True)
    ...
    if args.commit_files:
        os.chdir('gitian.sigs')
        commit = False
        if os.path.isfile(args.version+'-win-signed/'+args.signer+'/qrax-win-signer-build.assert.sig'):
            subprocess.check_call(['git', 'add', args.version+'-win-signed/'+args.signer])
            commit = True
        if os.path.isfile(args.version+'-osx-signed/'+args.signer+'/qrax-dmg-signer-build.assert.sig'):
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../qrax/contrib/gitian-descriptors/gitian-linux.yml']):
        print('Verifying v'+args.version+' Linux FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../qrax/contrib/gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../qrax/contrib/gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    # TODO: Skip checking signed windows sigs until we actually start producing signed windows binaries
    #print('\nVerifying v'+args.version+' Signed Windows\n')
    #if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../qrax/contrib/gitian-descriptors/gitian-win-signer.yml']):
    #    print('Verifying v'+args.version+' Signed Windows FAILED\n')
    #    rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../qrax/contrib/gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    if args.pull:
        ...
        if not os.path.isdir('../gitian-builder/inputs/qrax'):
            os.makedirs('../gitian-builder/inputs/qrax')
        os.chdir('../gitian-builder/inputs/qrax')
