...
*Setup instructions for a gitian build of QRAX Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the QRAX
...
to the QRAX GitHub release page.
...
- [Building QRAX Core](#building-qrax-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/qrax-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/qrax-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/qrax-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/qrax-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/qrax-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/qrax-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/qrax-labs/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@qrax.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/qrax-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/qrax-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/qrax-osx-*-build.assert
```
