...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/QRAXParams"
    else
        PARAMS_DIR="$HOME/.qrax-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
QRAX - install-params.sh

This script will install the QRAX zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common QRAX zkSNARK parameters. Note that it is
