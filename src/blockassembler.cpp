...
bool CreateCoinbaseTx(CBlock* pblock, const CScript& scriptPubKeyIn, CBlockIndex* pindexPrev, CWallet* pwallet)
{
    assert(pindexPrev);
    ...
    txNew.walletSenderId = pwallet->GetWalletIdentHash();
                                               ...
                                               std::vector<CStakeableOutput>* availableCoins)
{
    ...
    if (!(fProofOfStake ? SolveProofOfStake(pblock, pindexPrev, pwallet, availableCoins)
                        : CreateCoinbaseTx(pblock, scriptPubKeyIn, pindexPrev, pwallet))) {
        return nullptr;
    }
    ...
    if (fMultiMiningIsInit) {
        ...
        if (fProofOfStake) {
            CMutableTransaction multiminingTx;

			if (pwallet->CreateAssetRewardTransactions(multiminingTx)) {
                const CTransaction mTx(multiminingTx);
                pblock->vtx.emplace_back(std::make_shared<const CTransaction>(mTx));
                nBlockTx++;
				nBlockSize += mTx.GetTotalSize();
			}

        }
         std::map<uint160, std::pair<uint160, int16_t>> regs = multiMiningManager.GetRegistrations();
         std::map<uint160, std::pair<uint160, int16_t>>::const_iterator it = regs.cbegin();

         if (regs.size() > 0) {
             for (it = regs.cbegin(); it != regs.cend(); ++it)
             {
                 CAssetRegistration r;
                 r.owner = (*it).second.first;
                 r.key = (*it).first;
                 r.type = (*it).second.second;
                 pblock->vReg.emplace_back(r);
                 nBlockSize += r.GetSize();
                 if (nBlockSize >= nBlockMaxSize) break;
             }
             regs.erase(regs.cbegin(), it);
         }

		 if (NetworkUpgradeActive(nHeight, chainparams.GetConsensus(), Consensus::ACTIVATE_ASSET_VALIDATION)) {
			 pblock->assetRootHash = multiMiningManager.GetCurrentAssetHash();
		 }
    }
...
{
    if (NetworkUpgradeActive(nHeight, chainparams.GetConsensus(), Consensus::ACTIVATE_HUSH)) {
