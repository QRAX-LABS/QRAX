...
bool CheckBlockSignature(const CBlock& block)
{
	...
	/** Each block is signed by the private key of the input that is staked. This can be either normal UTXO
    ...
    if (whichType == TX_PUBKEY) {
        ...
        if (txin.scriptSig.size() == 73) { // Sig size + DER signature size.
            ...
            return false;
        } else if (whichType == TX_ASSETSMINT) {
            pubkey = CPubKey(txin.scriptSig.begin()+3, txin.scriptSig.end());
        } else {
