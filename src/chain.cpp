...
CBlockIndex::CBlockIndex(const CBlock& block):
		...
		assetRootHash{block.assetRootHash},
        nTime{block.nTime},
        nBits{block.nBits},
		nNonce{block.nNonce}
...
CBlockHeader CBlockIndex::GetBlockHeader() const
{
    ...
    block.nBits = nBits;
    block.nNonce = nNonce;
    if (nVersion >= 2) block.hashFinalSaplingRoot = hashFinalSaplingRoot;
	if (nVersion >= 3) block.assetRootHash = assetRootHash;
    return block;
}
...
int64_t CBlockIndex::MinPastBlockTime() const
{
    ...
    // so we allow the nBlockTimeProtocolV2 (QRAX v4.0) to be at most (180-14) seconds earlier than previous block
    if (nHeight + 1 == consensus.vUpgrades[Consensus::ACTIVATE_POS].nActivationHeight)
