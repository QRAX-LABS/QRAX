...
enum UpgradeIndex : uint32_t {
    BASE_NETWORK,
    ACTIVATE_POS,
	ACTIVATE_NEW_COLDSTAKE,
	ACTIVATE_NEW_FEE_RULES,
	ACTIVATE_ASSET_VALIDATION,
    ACTIVATE_HUSH,
...
struct Params {
    ...
    std::string strGenesisPubKeyHex;

    // spork keys
    std::string strSporkPubKey;

    // multimining keys
    std::string strMultiMiningProgramKey;
    ...
    bool IsTimeProtocolV2(const int nHeight) const { return NetworkUpgradeActive(nHeight, ACTIVATE_POS); }
    ...
    int FutureBlockTimeDrift(const int nHeight) const
    {
        ...
        return (NetworkUpgradeActive(nHeight, ACTIVATE_POS) ? nFutureTimeDriftPoS : nFutureTimeDriftPoW);
    }
