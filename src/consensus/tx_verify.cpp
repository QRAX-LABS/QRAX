...
bool CheckTransaction(const CTransaction& tx, CValidationState& state, bool fColdStakingActive, bool fSaplingActive)
{
    ...
    for (const CTxOut& txout : tx.vout) {
        if (txout.IsEmpty() && !tx.IsCoinBase() && !tx.IsCoinStake() && !tx.ContainsAssets())
        ...
        if (txout.scriptPubKey.IsPayToColdStaking()) {
            ...
            if (txout.nValue < MIN_COLDSTAKING_AMOUNT && chainActive.Tip()->nHeight > 21555)
    ...
    if (tx.IsCoinBase()) {
        ...
        for (const CTxIn& txin : tx.vin)
            if (txin.prevout.IsNull() && !txin.IsMultiMiningSpend())
