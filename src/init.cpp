...
#include "multimining/assetregistration.h"
#include "netbase.h"
...
void PrepareShutdown()
{
    ...
    util::ThreadRename("qrax-shutoff");
    ...
    DumpRegistrations(multiMiningManager);
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), QRAX_CONF_FILENAME));
	...
	strUsage += HelpMessageOpt("-paramsdir=<dir>", strprintf(_("Specify zk params directory (default: %s)"), GetParamsDir().string()));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), QRAX_PID_FILENAME));
#endif
    ...
    if (showDebug) {
        ...
        strUsage += HelpMessageOpt("-multiminingkey=<privkey>", _("Enable multimining program administration functionality with the appropriate private key."));
    }
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all QRAX specific functionality (Masternodes, Budgeting) (0-1, default: %u)"), 0));

    strUsage += HelpMessageGroup(_("Assets Configuration:"));
    strUsage += HelpMessageOpt("-multiwallets=<n>", strprintf(_("Allow each address to be identified as a unique node (0-1, default: %u)"), DEFAULT_MULTIWALLETS));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), QRAX_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:18962"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
            FormatParagraph(strprintf(_("Copyright (C) 2020-%i The QRAX Core Developers"), COPYRIGHT_YEAR)) + "\n" +
            "\n" +
...
void ThreadImport(const std::vector<fs::path>& vImportFiles)
{
    ...
    util::ThreadRename("qrax-loadblk");
    ...
    if (fReindex) {
        ...
        if (!LoadGenesisBlock()) {
			throw std::runtime_error("ThreadImport: Error initializing block database");
        }
    }
...
/** Sanity checks
 *  Ensure that QRAX is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single QRAX process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-multiminingkey")) {
        if (!mmProgramManager.SetPrivKey(gArgs.GetArg("-multiminingkey", "")))
            return UIError(_("Unable to sign multimining program message, wrong key?"));
    }
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (gArgs.GetBoolArg("-resync", false)) {
            ...
            fs::path multiminingDir = GetDataDir() / "multimining";

			LogPrintf("Deleting blockchain folders blocks, chainstate, sporks and multimining\n");
            ...
            try {
                ...
                if (fs::exists(multiminingDir)){
                    fs::remove_all(multiminingDir);
                    LogPrintf("-resync: folder deleted: %s\n", multiminingDir.string().c_str());
                }
            } catch (const fs::filesystem_error& error) {
    ...
    fs::create_directories(GetDataDir() / "multimining");
    ...
    LogPrintf("* Using %.1fMiB for Multimining Tree set\n", nMultiMiningCache * (1.0 / 1024 / 1024));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                mmProgramDB = new CMultiMiningProgramDB(0, false, false);

                multiMiningDB = new CMultiMiningDB(nMultiMiningCache, false, fReindex);
                ...
                // QRAX: load previous sessions sporks if we have them.
                uiInterface.InitMessage(_("Loading sporks..."));
                sporkManager.LoadSporksFromDB();

                uiInterface.InitMessage(_("Loading programs..."));
                mmProgramManager.LoadProgramsFromDB();
                ...
                if (!fReindex && !LoadGenesisBlock()) {
					strLoadError = _("LoadGenesisBlock: Error initializing block database");
                    break;
                }
                ...
                if (fMultiWalletsNode != gArgs.GetBoolArg("-multiwallets", DEFAULT_MULTIWALLETS)) {
                    strLoadError = _("You need to rebuild the database using -reindex to change -multiwallets");
                    break;
                }
                ...
                if (!fReindex) {
                    ...
                    if (!LoadChainTip(chainparams)) {
						strLoadError = _("LoadChainTip: Error initializing block database");
                        break;
                    }
...
// ********************************************************* Step 8: load wallet
    ...
    {
        LogPrintf("MultiWallets Mode Activated!!!\n");
		/*if (gArgs.SoftSetBoolArg("-staking", false))
			LogPrintf("AppInit2 : parameter interaction: wallet functionality not enabled -> setting -staking=0\n");*/
    }
    ...
    {
        if (pwalletMain) {
            fMultiMiningIsInit = multiMiningManager.InitManager();
        }
    }
    #endif
    ...
    CAssetRegistrationDB assetsdb;
    //if (!fDryRun) g_budgetman.SetBestHeight(nChainHeight);
    std::map<uint160, std::pair<uint160, int16_t>> regs;
    CAssetRegistrationDB::ReadResult assetReadResult = assetsdb.Read(regs, fDryRun);

    if (assetReadResult == CAssetRegistrationDB::FileError)
        LogPrintf("Missing asset registrations cache - assetregs.dat, will try to recreate\n");
    else if (assetReadResult != CAssetRegistrationDB::Ok) {
        LogPrintf("Error reading assetregs.dat: ");
        if (assetReadResult == CAssetRegistrationDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }

    if (fMultiMiningIsInit) multiMiningManager.InsertRegistrations(regs);
    ...
    if (pwalletMain) {
        ...
        if (fMultiMiningIsInit) threadGroup.create_thread(boost::bind(&ThreadMultimining));
    }
#endif
