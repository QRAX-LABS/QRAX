...
std::unique_ptr<CBlockTemplate> CreateNewBlockWithKey(CReserveKey* reservekey, CWallet* pwallet)
{
    ...
    if (Params().GetConsensus().NetworkUpgradeActive(nHeightNext, Consensus::ACTIVATE_POS)) {
...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("QRAX Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("QRAX Miner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("QRAX Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("qrax-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            if (!consensus.NetworkUpgradeActive(pindexPrev->nHeight + 1, Consensus::ACTIVATE_POS)) {
            ...
            while ((g_connman && g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && Params().MiningRequiresPeers())
					|| pwallet->IsLocked() || !fStakeableCoins || masternodeSync.NotCompleted() || IsInitialBlockDownload()) {
                MilliSleep(5000);
        ...
        } else if (pindexPrev->nHeight > 6 && consensus.NetworkUpgradeActive(pindexPrev->nHeight - 6, Consensus::ACTIVATE_POS)) {
        ...
        LogPrintf("Running QRAX Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("QRAX Miner exception: %s\n", e.what());
    } catch (...) {
        LogPrintf("QRAX Miner exception\n");
    }

    LogPrintf("QRAX Miner exiting\n");
}
