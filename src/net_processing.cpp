...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    case MSG_MMPROGRAM:
        return mapPrograms.count(inv.hash);
                                      ...
                                      CNetMsgMaker& msgMaker)
{
    ...
    if (inv.type == MSG_MMPROGRAM) {
        if (mapPrograms.count(inv.hash)) {
            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss.reserve(1000);
            ss << mapPrograms[inv.hash];
            connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::MMPROGRAM, ss));
            return true;
        }
    }

    if (inv.type == MSG_MULTIMINING_WID) {
        return true;
    }
...
bool fRequestedMultiMiningPrograms = false;
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // QRAX: We use certain sporks during IBD, so check to see if they are
        ...
        if (!fRequestedMultiMiningPrograms && fMultiMiningIsInit) {
            LogPrintf("%s: asking peer for multimining programs = %s\n", __func__, fRequestedMultiMiningPrograms ? "true" : "false");
            connman.PushMessage(pfrom, CNetMsgMaker(nSendVersion).Make(NetMsgType::GETMMPROGRAMS));
            fRequestedMultiMiningPrograms = true;
        }
    ...
    // getaddr message mitigates the attack.
        ...
        if (found) {
            ...
            if (fMultiMiningIsInit) multiMiningManager.ProcessNetMessage(pfrom, strCommand, vRecv);
            mmProgramManager.ProcessProgram(pfrom, strCommand, vRecv);
