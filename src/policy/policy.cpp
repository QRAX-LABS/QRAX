...
{
    return (txout.nValue < GetDustThreshold(dustRelayFee));
}
...
bool IsStandardTx(const CTransactionRef& tx, int nBlockHeight, std::string& reason)
{
    ...
    for (const CTxOut& txout : tx->vout) {
        bool fBurnAmountTransaction = false;
        uint160 curhash;
        if (txout.scriptPubKey.IsPayToScriptHash()) {
            std::vector<unsigned char> hashBytes(txout.scriptPubKey.begin()+2, txout.scriptPubKey.begin()+22);
            curhash = uint160(hashBytes);
        } else if (txout.scriptPubKey.IsPayToPublicKeyHash()) {
            std::vector<unsigned char> hashBytes(txout.scriptPubKey.begin()+3, txout.scriptPubKey.begin()+23);
            curhash = uint160(hashBytes);
        } else if (txout.scriptPubKey.IsPayToPublicKey()) {
            curhash = uint160(Hash160(txout.scriptPubKey.begin()+1, txout.scriptPubKey.end()-1));
        }
        ...
        if (whichType == TX_NULL_DATA)
            ...
            return false;
        } else if (!fBurnAmountTransaction && IsDust(txout, ::minRelayTxFee) && !txout.IsMultiMiningMint()) {
            reason = "dust";
            return false;
        }
    }
