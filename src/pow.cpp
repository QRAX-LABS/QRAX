...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock)
{
    ...
    /* current difficulty formula, qrax - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
    ...
    if (consensus.NetworkUpgradeActive(pindexLast->nHeight + 1, Consensus::ACTIVATE_POS)) {
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
    ...
    if (hash > bnTarget)
        return error("CheckProofOfWork() : hash doesn't match nBits (%s = %s) is %d", hash.ToString().c_str(), bnTarget.ToString().c_str(), hash > bnTarget );
