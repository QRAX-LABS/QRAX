...
static QString translateTypeToString(AddressTableEntry::Type type)
{
    switch (type) {
        ...
        case AddressTableEntry::ShieldedReceive:
            return QObject::tr("Receiving Hush");
        case AddressTableEntry::ShieldedSend:
            return QObject::tr("Contact Hush");
...
public:
    ...
    // add hush addresses num if needed..
...
void AddressTableModel::updateEntry(const QString& address,
    ...
    int status)
{
	// Update address book model from Qrax core
...
QString AddressTableModel::getAddressToShow(bool isShielded) const
{
    ...
    for (auto it = wallet->NewAddressBookIterator(); it.IsValid(); it.Next()) {
        ...
        if (!isShielded) {
            ...
            // todo: add hush address support to IsUsed
    ...
    if (!isShielded) {
        ...
        res = walletModel->getNewShieldedAddress(addressStr, "default hush");
    }
    return addressStr;
}
