...
AssetPercents::AssetPercents(QObject* parent) : QAbstractListModel(parent),
                                              percentlist(availablePercents())
{
}

QList<AssetPercents::Percent> AssetPercents::availablePercents()
{
    QList<AssetPercents::Percent> plist;
    plist.append(Day);
    plist.append(Month);
    plist.append(Year);
    return plist;
...
QString AssetPercents::name(int unit)
{
    switch (unit) {
    case Day:
        return tr("Day");
...
QVariant AssetPercents::data(const QModelIndex& index, int role) const
{
    int row = index.row();
    if (row >= 0 && row < percentlist.size()) {
        Percent p = percentlist.at(row);
        switch (role) {
        case Qt::EditRole:
        case Qt::DisplayRole:
            return QVariant(name(p));
        case Qt::ToolTipRole:
            return QVariant(name(p));
        case PercentRole:
            return QVariant(static_cast<int>(p));
        }
    }
    return QVariant();
}
...
int AssetPercents::rowCount(const QModelIndex& parent) const
{
    Q_UNUSED(parent);
...
QList<BitcoinUnits::Unit> BitcoinUnits::availableUnits()
{
    ...
    unitlist.append(QRAX);
    unitlist.append(mQRAX);
    unitlist.append(uQRAX);
    return unitlist;
}
...
QString BitcoinUnits::id(int unit)
{
    ...
    case QRAX:
        return QString("QRAX");
    case mQRAX:
        return QString("mQRAX");
    case uQRAX:
        return QString::fromUtf8("uQRAX");
    default:
        return QString("???");
    }
}
