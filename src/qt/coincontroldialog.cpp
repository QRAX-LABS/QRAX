...
TotalAmounts CoinControlDialog::getTotals() const
{
    ...
    if (t.nQuantity > 0) {
        ...
        // Hush txes must include binding sig and valueBalance
        bool isShieldedTx = (nShieldIns + nShieldOuts > 0);
        if (isShieldedTx) {
            t.nBytes += (BINDINGSIG_SIZE + 8);
            // hush in/outs len sizes
...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select QRAX Outputs to Spend" :
            "Select Hush QRAX to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::QRAX;
										  ...
										  const bool isLockedCoin)
{
    ...
    // if listMode or change => show QRAX address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
