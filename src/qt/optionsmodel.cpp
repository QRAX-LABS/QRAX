...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::QRAX);
    nDisplayUnit = settings.value("nDisplayUnit").toInt();

    if (!settings.contains("nAssetPercentPeriod") || reset)
        settings.setValue("nAssetPercentPeriod", AssetPercents::Day);
    nAssetPercentPeriod = settings.value("nAssetPercentPeriod").toInt();
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in qrax.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case nCustomFee:
            return QVariant(static_cast<qlonglong>((pwalletMain) ? pwalletMain->nCustomFee : ::minRelayTxFee.GetPercentFee(0)));
#endif
        case DisplayUnit:
            return nDisplayUnit;
        ...
        case AssetPercentPeriod:
            return nAssetPercentPeriod;
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AssetPercentPeriod:
            if (!value.isNull()) {
                QSettings settings;
                nAssetPercentPeriod = value.toInt();
                settings.setValue("nAssetPercentPeriod", nAssetPercentPeriod);
                Q_EMIT changeDisplayAssetPercent(nAssetPercentPeriod);
            }
            break;
        default:
            break;
