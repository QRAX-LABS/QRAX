...
#include "qt/qrax/splash.h"
#include "qt/qrax/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("qrax-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in qrax.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in qrax.qrc)
...
/** Class encapsulating QRAX Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (QRAXGUI)
...
{
    window = new QRAXGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &QRAXGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &QRAXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // QRAX: URIs or payment requests:
	//connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &QRAXGUI::handlePaymentRequest);
	connect(window, &QRAXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. QRAX can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(qrax_locale);
    Q_INIT_RESOURCE(qrax);
    ...
    /// 6. Determine availability of data directory and parse qrax.conf
	...
	QMessageBox::critical(0, QObject::tr("QRAX Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // qrax: links repeatedly have their payment requests routed to this process:
