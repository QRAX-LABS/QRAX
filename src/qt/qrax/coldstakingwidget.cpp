...
#include "qt/qrax/forms/ui_coldstakingwidget.h"
#include "qt/qrax/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/qrax/requestdialog.h"
#include "qt/qrax/tooltipmenu.h"
#include "qt/qrax/furlistrow.h"
...
ColdStakingWidget::ColdStakingWidget(QRAXGUI* parent) :
    PWidget(parent),
...
void ColdStakingWidget::loadWalletModel()
{
    if (walletModel) {
		...
		csModel = new ColdStakingModel(walletModel, walletModel->getAddressTableModel(), this);
		...
		connect(txModel, &InternalTransactionTableModel::txArrived, this, &ColdStakingWidget::onTxArrived);
...
void ColdStakingWidget::onDelegateSelected(bool delegate)
{
    ...
    if (isInDelegation) {
        ...
        //ui->btnListDposNodes->setVisible(true);
        ui->emptyContainer->setVisible(false);
        ui->listView->setVisible(false);
        //ui->listViewDposNodes->setVisible(isDposListVisible);
        ui->containerHistoryLabel->setVisible(false);
