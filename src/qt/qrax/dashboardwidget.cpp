...
#include "qt/qrax/forms/ui_dashboardwidget.h"
#include "qt/qrax/sendconfirmdialog.h"
#include "qt/qrax/txrow.h"
...
DashboardWidget::DashboardWidget(QRAXGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareQrax, "square-chart-qrax");
	setCssProperty(ui->labelSquareAsset, "square-chart-asset");
	setCssProperty(ui->labelQrax, "text-chart-qrax");
	setCssProperty(ui->labelAsset, "text-chart-asset");

    // Staking Amount
    QFont fontBold;
    fontBold.setWeight(QFont::Bold);

    //setCssProperty({ui->labelStaking, ui->labelMultimining}, "legend-chart");
...
void DashboardWidget::changeSort(int nSortIndex)
{
	...
	//Q_EMIT updateSortType(nSortIndex);
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair Qrax, Asset
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
			...
			qrax = (pair.first != 0) ? pair.first / 100000000 : 0;
			asset = (pair.second != 0) ? pair.second / 100000000 : 0;
			chartData->totalQrax += pair.first;
			chartData->totalAsset += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesQrax.append(qrax);
		chartData->valuesAsset.append(asset);

		int max = std::max(qrax, asset);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set1 = new QBarSet(CURRENCY_UNIT.c_str());
    set0->setColor(QColor(15, 34, 176));
    set1->setColor(QColor(97, 114, 242));
	...
	set0->append(chartData->valuesQrax);
	set1->append(chartData->valuesAsset);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalQrax > 0 || chartData->totalAsset > 0) {
		setCssProperty(ui->labelAmountQrax, "text-stake-qrax");
		setCssProperty(ui->labelAmountAsset, "text-stake-asset");
    } else {
		setCssProperty(ui->labelAmountQrax, "text-stake-qrax-disable");
		setCssProperty(ui->labelAmountAsset, "text-stake-asset-disable");
    }
	forceUpdateStyle({ui->labelAmountQrax, ui->labelAmountAsset});

	ui->labelAmountQrax->setText(tr("Staking: %1").arg(GUIUtil::formatBalance(chartData->totalQrax, nDisplayUnit)));
	ui->labelAmountAsset->setText(tr("Assets: %1").arg(GUIUtil::formatBalance(chartData->totalAsset, nDisplayUnit)));
