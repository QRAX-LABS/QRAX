...
#include "config/qrax-config.h" /* for USE_QTCHARTS */
#endif
...
enum SortTx {
	DATE_DESC = 0, // newest to oldest
	DATE_ASC = 1, // oldest to newest
    AMOUNT_DESC = 2,
    AMOUNT_ASC = 3
};
...
public:
	...
	qint64 totalQrax = 0;
	qint64 totalAsset = 0;
	QList<qreal> valuesQrax;
	QList<qreal> valuesAsset;
    QStringList xLabels;
};
...
public:
    explicit DashboardWidget(QRAXGUI* _window);
    ~DashboardWidget();
