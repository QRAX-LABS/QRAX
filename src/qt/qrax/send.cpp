...
#include "qt/qrax/forms/ui_send.h"
#include "qt/qrax/addnewcontactdialog.h"
#include "qt/qrax/qtutils.h"
#include "qt/qrax/sendchangeaddressdialog.h"
#include "qt/qrax/optionbutton.h"
...
SendWidget::SendWidget(QRAXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnShieldCoins->setSubTitleClassAndText("text-subtitle", tr("Convert all transparent coins into hush coins"));
	...
	setCssProperty(coinIcon, "coin-icon-qrax");
    ...
    // Connect
	connect(ui->pushLeft, &QPushButton::clicked, [this](){onCoinSelected(true);});
	//connect(ui->pushRight,  &QPushButton::clicked, [this](){onCoinSelected(false);});
...
void SendWidget::refreshAmounts()
{
    ...
    QString type = isTransparent ? "transparent" : "hush";
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent QRAXs.
                             ...
                             const std::function<bool(QList<SendCoinsRecipient>&)>& func)
{
    ...
    if (isShieldedTx) {
        if (!walletModel->isSaplingEnforced()) {
            inform(tr("Cannot perform hush operations, upgrade isn't being enforced yet!"));
            return;
        }

        if (walletModel->isSaplingInMaintenance()) {
            inform(tr("Hush Protocol temporarily in maintenance. Hush transactions disabled"));
            return;
        }
    }
...
bool SendWidget::sendFinalStep()
{
    ...
    if (ptrModelTx->fBurnAmountTransaction)
        warningStr = warningStr + "\n" + tr("WARNING:\nSending coins to Genesis Address.\n"
                        "All sended coins will be burned.");
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        // (plus at least 2 bytes for hush in/outs len sizes)
        nBytesInputs += 2;
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield QRAXs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent QRAXs to shield."));
    }
}
...
void SendWidget::onCoinSelected(bool _isTransparent)
{
