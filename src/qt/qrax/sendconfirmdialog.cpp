...
#include "qt/qrax/forms/ui_sendconfirmdialog.h"
    ...
    isConfirmDialog(_isConfirmDialog)
{
    ...
    setCssProperty({ui->labelAmount, ui->labelSend, ui->labelInputs, ui->labelFee, ui->labelChange, ui->labelId, ui->labelToSend, ui->labelStatus, ui->labelConfirmations, ui->labelDate, ui->labelMemo}, "text-subtitle");
    ...
    setCssProperty({ui->textAmount, ui->textSendLabel, ui->textInputs, ui->textFee, ui->textChange, ui->textId, ui->textToSend, ui->textStatus, ui->textConfirmations, ui->textDate, ui->textMemo} , "text-body3-dialog");
    ...
    if (isConfirmDialog) {
        ...
        //ui->contentSize->setVisible(true);
...
void TxDetailDialog::setData(WalletModel *_model, const QModelIndex &index)
{
    ...
    if (_tx) {
        ...
        // future: subdivide hush and transparent by type and
        ...
        CAmount nFee = (_tx->IsFromMe(filter) ? model->GetFeeByValue(rec->hash, rec->getOutputIndex()) : 0);
        ui->textFee->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, nFee, false, BitcoinUnits::separatorAlways));

        CAmount nToSend = index.data(TransactionTableModel::AmountRole).toLongLong();;
        ui->textToSend->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, nToSend, false, BitcoinUnits::separatorAlways));

        qint64 amount = nToSend - nFee;
...
void TxDetailDialog::setData(WalletModel *_model, WalletModelTransaction* _tx)
{
    ...
    ui->textToSend->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, totalAmount - txFee, false, BitcoinUnits::separatorAlways));
