...
#include "qt/qrax/settings/forms/ui_settingsdisplayoptionswidget.h"
#include <QListView>
#include <QSettings>
#include <QDir>
#include "guiutil.h"
...
SettingsDisplayOptionsWidget::SettingsDisplayOptionsWidget(QRAXGUI* _window, QWidget *parent) :
    ...
    ui(new Ui::SettingsDisplayOptionsWidget)
{
    ...
    ui->labelAssetPercent->setProperty("cssClass", "text-main-settings");
    ...
    ui->comboBoxAssetUnit->setProperty("cssClass", "btn-combo");
    ui->comboBoxAssetUnit->setView(new QListView());
    ui->comboBoxAssetUnit->setModel(new AssetPercents(this));
    ui->comboBoxAssetUnit->setModelColumn(Qt::DisplayRole);
    ui->comboBoxAssetUnit->setEditable(true);

    QLineEdit* AssetUnitEdit = new QLineEdit(ui->comboBoxAssetUnit);
    AssetUnitEdit->setReadOnly(true);
    AssetUnitEdit->setAlignment(Qt::AlignRight);
    ui->comboBoxAssetUnit->setLineEdit(AssetUnitEdit);
...
void SettingsDisplayOptionsWidget::setMapper(QDataWidgetMapper *mapper)
{
    ...
    mapper->addMapping(ui->comboBoxAssetUnit, OptionsModel::AssetPercentPeriod);
#ifdef USE_QTCHARTS
...
void SettingsDisplayOptionsWidget::loadClientModel()
{
    if (clientModel) {
        ...
        ui->comboBoxAssetUnit->setCurrentIndex(this->clientModel->getOptionsModel()->getAssetPercentPeriod());
        initLanguages();
    }
}
