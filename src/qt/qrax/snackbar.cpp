...
#include "qt/qrax/forms/ui_snackbar.h"
#include "qt/qrax/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(QRAXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &QRAXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
