...
#include "qt/qrax/txviewholder.h"
...
void TxViewHolder::init(QWidget* holder, const QModelIndex &index, bool isHovered, bool isSelected) const
{
    ...
    qint64 amountFee = rIndex.data(TransactionTableModel::FeeAmountRole).toLongLong();
    QString amountText = BitcoinUnits::formatWithUnit(nDisplayUnit, amountTop - amountFee, true, BitcoinUnits::separatorAlways);
    ...
    if (hasDoubleAmount) {
        ...
        txRow->setAmount(amountBottomText + (type == TransactionRecord::SendToSelfShieldedAddress ? " hush" : ""),
                         amountText + " fee");
    } else {
