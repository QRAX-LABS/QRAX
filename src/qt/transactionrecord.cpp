...
bool TransactionRecord::decomposeMultiMiningReward(const CWallet* wallet, const CWalletTx& wtx,
                                                   const CAmount& nCredit, const CAmount& nDebit,
                                                   QList<TransactionRecord>& parts)
{
    // Return if it's not a coinstake
    if (!wtx.ContainsAssets()) {
        return false;
    }
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
		...
		// a that was sent to an address held by this wallet
        isminetype mine = wallet->IsMine(txout);
        if (mine) {
            TransactionRecord sub(hash, nTime, wtx.tx->GetTotalSize());
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            sub.type = txout.IsAssetDposMint() ? TransactionRecord::DposAssetReward : TransactionRecord::AssetReward; // MultiMiningReward
                                                       ...
                                                       QList<TransactionRecord>& parts, const CWallet* wallet)
{
    ...
    if (!wtx.tx->hasSaplingData()) {
        ...
        // we know that all of the inputs and outputs are mine and that have hush data.
        ...
        if (wtx.tx->sapData->vShieldedSpend.empty()) {
            ...
            // we know that the inputs are hush now, let's see if
    ...
    //sub.feeAmount = wallet->GetFeeByAmount(nCredit - nDebit);
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned QRAXs, op_return could be for a proposal/budget fee or another sort of data stored there.
...
{
    // Check if all the hush spends are from me
    ...
    // Check if all the hush outputs are to me
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (parts.empty()) {
		...
		TransactionRecord record(wtx.GetHash(), wtx.GetTxTime(), wtx.tx->GetTotalSize(), TransactionRecord::Other, "", nNet, 0);
...
void TransactionRecord::updateStatus(const CWalletTx& wtx, int chainHeight)
{
    ...
    int nBlocksToMaturity = (wtx.IsCoinBase() || wtx.IsCoinStake() || wtx.ContainsAssets()) ? std::max(0, (Params().GetConsensus().nCoinbaseMaturity + 1) - depth) : 0;
...
{
	return (type == TransactionRecord::AssetReward || type == TransactionRecord::DposAssetReward || type == TransactionRecord::StakeMint || type == TransactionRecord::Generated);
}
