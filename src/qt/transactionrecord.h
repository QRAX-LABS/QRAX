...
public:
    enum Type {
        ...
        SendToShielded, // Hush send
        RecvWithShieldedAddress, // Hush receive
        SendToSelfShieldedAddress, // Hush send to self
        SendToSelfShieldToTransparent, // Unshield coins to self
        SendToSelfShieldToShieldChangeAddress, // Changing coins from one hush address to another inside the wallet.
        AssetReward,
        DposAssetReward,
        SendToNobody // Burned QRAXs, op_return output.
    };
    ...
    static bool decomposeMultiMiningReward(const CWallet* wallet, const CWalletTx& wtx,
                                           const CAmount& nCredit, const CAmount& nDebit,
                                           QList<TransactionRecord>& parts);
