...
#include "qt/qrax/dashboardwidget.h"
...
public:
	...
	std::vector<CWalletTx> loadedWalletTxes;

	unsigned int txTotalCount;
    ...
    void updateWallet(const uint256& hash, int status, bool showTransaction, TransactionRecord& ret)
    {
		...
		qDebug() << "TransactionTablePriv::updateWallet : inModel=" + QString::number(inModel) +
	...
	void insertMoreRows(unsigned int count)
	{
		...
		std::vector<TransactionRecord> cached = std::vector<TransactionRecord>(cachedWallet.begin(), cachedWallet.end());
		sort(cached.begin(), cached.end(),
				[](const TransactionRecord & a, const TransactionRecord & b) -> bool {
				 return a.time > b.time;
			 });

		// last transaction
		const TransactionRecord lastRow = cached.back();
		uint256 lastHash = lastRow.hash;

		unsigned int index = 0;

		for (unsigned int i = 0; i < walletTxes.size(); i++)
		{
			const CWalletTx* tx = &walletTxes[i];
			if (lastHash == tx->GetHash()) {
				index = i + 1;
				break;
			}
		}
		...
		// Now take the remaining ones and do the work here
		std::size_t const remainingSize = count - totalSumSize;
...
{
	Q_UNUSED(parent);
	return priv->size() < priv->getTotalCount();
}

void TransactionTableModel::fetchMore(const QModelIndex &parent)
{

	int remainder =	priv->getTotalCount() - priv->size();
	int itemsToFetch = qMin(10, remainder);

	if (itemsToFetch <= 0)
		return;

	beginInsertRows(QModelIndex(), priv->size(), priv->size() + itemsToFetch - 1);
	priv->insertMoreRows(itemsToFetch);
	endInsertRows();
...
public:
	...
	TransactionNotification(uint256 hash, ChangeType status, int showTransaction) : hash(hash), status(status), showTransaction(showTransaction) {}

    void invoke(QObject* ttm)
    {
        QString strHash = QString::fromStdString(hash.GetHex());
		qDebug() << "TransactionTableModel::NotifyTransactionChanged : " + strHash + " status= " + QString::number(status) + ", showTransaction= " + (showTransaction == 1 ? "yes" : "no");
...
static void NotifyTransactionChanged(TransactionTableModel* ttm, const uint256& hash, ChangeType status, int showTransaction)
{
