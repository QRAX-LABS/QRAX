																						 ...
																						 m_cached_have_locked_tx(false)
{
	...
	internalTransactionTableModel = new InternalTransactionTableModel(wallet, this);
	//transactionTableModel = new TransactionTableModel(wallet, this);
    //recentRequestsTableModel = new RecentRequestsTableModel(wallet, this);
    assetRewardHistoryTableModel = new AssetRewardsHistoryTableModel(wallet, this);
...
void WalletModel::init()
{
	...
	internalTransactionTableModel->init();
...
{
    return Params().GetConsensus().NetworkUpgradeActive(cachedNumBlocks, Consensus::ACTIVATE_HUSH);
}

bool WalletModel::isNewFeeActivated() const
{
	return Params().GetConsensus().NetworkUpgradeActive(cachedNumBlocks, Consensus::ACTIVATE_NEW_FEE_RULES);
}
...
static bool processBalanceChangeInternal(WalletModel* walletModel)
{
	...
	walletModel->checkAssetsChanged(walletModel->getAssets());
	walletModel->setHasLockedCoins(walletModel->listLockedCoins().size());
...
{
	if (internalTransactionTableModel) {
		internalTransactionTableModel->updateConfirmations();
    }
}
...
void WalletModel::emitAssetsChanged()
{
   Q_EMIT multiminingStructureChanged();
    // Force update of UI elements even when no values have changed
   Q_EMIT multiminingChanged(walletWrapper.getAssets());
}
...
void WalletModel::checkAssetsChanged(const interfaces::AssetsBalance &newBalance)
{
    if (newBalance.balanceChanged(m_cached_assets))
    {
        m_cached_assets = newBalance;
        QMetaObject::invokeMethod(this, "assetsNotify", Qt::QueuedConnection);
    }
}

void WalletModel::assetsNotify()
{
    Q_EMIT multiminingStructureChanged();
    Q_EMIT multiminingChanged(m_cached_assets);
}
...
{
    // Only regular base58 addresses and hush addresses accepted here
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction* transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
    ...
    for (const SendCoinsRecipient& rcp : recipients) {
		{ // User-entered qrax address / amount:
...

    {
        ...
        bool fCreated = wallet->CreateTransaction(vecSend,
                                                  ...
                                                  *keyChange,
                                                  ...
                                                  &transaction->fBurnAmountTransaction);
		...
		bool isNewFee = isNewFeeActivated();
        // reject insane fee
		if (nFeeRequired > ::minRelayTxFee.GetPercentFee(total, isNewFee))
            return InsaneFee;
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
    ...
    bool fSaplingActive = Params().GetConsensus().NetworkUpgradeActive(cachedNumBlocks, Consensus::ACTIVATE_HUSH);
                                                        ...
                                                        const CCoinControl* coinControl)
{
    ...
    for (const auto& recipient : modelTransaction->getRecipients()) {
        if (recipient.isShieldedAddr) {
            ...
            if (!pa) return errorOut("Error, invalid hush address");
...
CAmount WalletModel::GetFeeByValue(uint256 id, unsigned int n) const
{
    const CWalletTx *tx = wallet->GetWalletTx(id);
    if (tx == NULL) return 0;
    return wallet->GetFeeByAmount(tx->tx->vout[n].nValue);
}
...
{
	return nullptr; //transactionTableModel;
}

InternalTransactionTableModel* WalletModel::getInternalTransactionTableModel()
{
	return internalTransactionTableModel;
}

AssetRewardsHistoryTableModel* WalletModel::getAssetRewardTableModel()
{
    return assetRewardHistoryTableModel;
}
...
*/
CAssetNode *WalletModel::GetMultiMiningOwnStructure()
{
    return wallet->GetMultiMiningStructureTree();
}

CAssetNode *WalletModel::GetInformationNode(const CKeyID &key)
{
    return wallet->GetAssetNodeInformation(key);
}
...
static void NotifyTransactionChanged(WalletModel* walletmodel, const uint256& hash, ChangeType status, int showTransaction)
{
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (isStaking)
        return error("Invalid QRAX address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CTxDestination& address, CKeyID& keyID)
{
    if (!IsValidDestination(address))
        return error("Invalid QRAX address");

    const CKeyID* inKeyID = boost::get<CKeyID>(&address);
    if (!inKeyID)
        return error("Unable to get KeyID from QRAX address");
...
// returns a COutPoint of 10000 QRAX if found
...
void WalletModel::listAvailableNotes(std::map<ListCoinsKey, std::vector<ListCoinsValue>>& mapCoins) const
{
    ...
    for (const auto& note : notes) {
        ...
        ListCoinsValue value{
            ...
            0,
			note.confirmations,
			wallet->IsLockedCoin(note.op.hash, note.op.n)
        };
...
void WalletModel::listCoins(std::map<ListCoinsKey, std::vector<ListCoinsValue>>& mapCoins) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        ListCoinsValue value{
                ...
                out.tx->GetTxTime(),
				out.nDepth,
				wallet->IsLockedCoin(out.tx->GetHash(), out.i)
        };
        mapCoins[key].emplace_back(value);
    }
}
