...
class InternalTransactionTableModel;
class WalletModelTransaction;
class AssetRewardsHistoryTableModel;
...
/** Interface to QRAX wallet from Qt view code. */
...
public:
	...
	InternalTransactionTableModel* getInternalTransactionTableModel();
    //RecentRequestsTableModel* getRecentRequestsTableModel();
    AssetRewardsHistoryTableModel* getAssetRewardTableModel();
    CAssetNode *GetMultiMiningOwnStructure();
    ...
    interfaces::AssetsBalance GetAssetBalances() {return m_cached_assets;};
    ...
    // Check address for validity and type (whether cold staking address or not),
    // plus return isShielded = true if the parsed address is a valid hush address.
    ...
    CAssetNode *GetInformationNode(const CKeyID &key);
    CAmount GetFeeByValue(uint256 id, unsigned int n) const;
...
private:
    ...
    AssetRewardsHistoryTableModel* assetRewardHistoryTableModel;

    // Cache balance to be able to detect changes
    interfaces::WalletBalances m_cached_balances;
    interfaces::AssetsBalance m_cached_assets;
...
Q_SIGNALS:
    ...
    void multiminingChanged(const interfaces::AssetsBalance& multiMiningBalance);
    void multiminingStructureChanged();
