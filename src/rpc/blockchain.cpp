...
UniValue getrawmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in qrax\n"
...
UniValue getblocksupply(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getblocksupply start (end)\n"
            ...
            "\nArguments:\n"
            "1. start       (int, require) start block to get supply\n"
            "2. end       (int, optional, default=latest) supply at height\n"

            "\nResult:\n"
            "{\n"
            "  \"nHeight\" : n,       (numeric) The chain height when the supply was updated\n"
            "  \"totalSupply\": n          (numeric) The transparentsupply\n"
            "}\n"

            "\nExamples:\n" +
            HelpExampleCli("getblocksupply", "") +
            HelpExampleRpc("getblocksupply", "3"));


    LOCK(cs_main);
    int startHeight = chainActive.Height();
    if (request.params.size() == 1 && startHeight >= request.params[0].get_int())
        startHeight = request.params[0].get_int();

    CAmount total = 0;
    GetSupplyIndexByHeight(total, startHeight);

    UniValue ret(UniValue::VOBJ);
    ret.pushKV("nHeight", startHeight);
    ret.pushKV("totalSupply", ValueFromAmount(total));
...
UniValue getsupplyinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "  \"totalsupply\": n          (numeric) The sum of transparentsupply\n"
            "}\n"
    ...
    ret.pushKV("totalsupply", ValueFromAmount(tSupply));
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"value\" : x.xxx,           (numeric) The transaction value in QRAX\n"
            "  \"scriptPubKey\" : {         (json object)\n"
            "     \"asm\" : \"code\",       (string) \n"
            "     \"hex\" : \"hex\",        (string) \n"
            "     \"reqSigs\" : n,          (numeric) Number of required signatures\n"
            "     \"type\" : \"pubkeyhash\", (string) The type, eg pubkeyhash\n"
            "     \"addresses\" : [          (array of string) array of qrax addresses\n"
...
UniValue getblockindexstats(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
                ...
                "  \"txcount\": xxxxx                (numeric) standart tx count\n"
                "  \"txcount_base\": xxxxx           (numeric) coinbase tx count\n"
                "  \"txcount_stake\": xxxxx          (numeric) coinstake tx count\n"
                "  \"txcount_asset\": xxxxx          (numeric) coinassets tx count\n"
    ...
    CAmount nFees = 0;
    int64_t nBytes = 0;
    int64_t nTxCount = 0; // standart tx
    int64_t nTxCoinBase = 0;
    int64_t nTxCoinStake = 0;
    int64_t nTxCoinAsset = 0;
    ...
    ret.pushKV("txcount_base", (int64_t)nTxCoinBase);
    ret.pushKV("txcount_stake", (int64_t)nTxCoinStake);
    ret.pushKV("txcount_asset", (int64_t)nTxCoinAsset);
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "blockchain",         "getblocksupply",         &getblocksupply,         true  },
