...
UniValue deletemasternode(const JSONRPCRequest& request)
{

	if (request.fHelp || request.params.empty() || request.params.size() != 3) {
		throw std::runtime_error(
			"deletemasternode alias hash index\n"
			"\nDelete masternode by alias\n"

			"\nArguments:\n"
			"1. alias\n"
			"2. tx collateral hash\n"
			"3. collateral out index\n"
			...
			"\nExamples:\n" +
			HelpExampleCli("deletemasternode", "\"mn1\" \"txhash\" 0") + HelpExampleRpc("deletemasternode", "mn1"));
	}

	std::string aliasToRemove = request.params[0].get_str();

	if (aliasToRemove.empty()) {
		return "error alias";
	}
	...
	if (fs::exists(pathBootstrap)) {
		...
		if (!streamConfig.good()) {
			return "Invalid masternode.conf file";
		}
		...
		for (std::string line; std::getline(streamConfig, line); linenumber++) {
			...
			if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
				...
				if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
					streamConfig.close();
					return "Error parsing masternode.conf file";
				}
			}
		...
		if (lineCopy.size() == 0) {
			lineCopy = "# Masternode config file\n"
									...
									"# Example: mn1 127.0.0.2:18962 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
		}
		...
		if (lineNumToRemove != -1) {
			...
			if (convertOK) {
				COutPoint collateralOut(uint256(hash), index);
				LOCK(pwalletMain->cs_wallet);
				pwalletMain->UnlockCoin(collateralOut);
			}
		...
		return "masternode.conf file doesn't exists";
	}
...
UniValue createmasternode(const JSONRPCRequest& request)
{
	...
	std::string strMasterNodeAddr = request.params[1].get_str();

	if (strMasterNodeAddr.empty()) {
		return "error address";
	}

	std::string strMasterNodePrivKey = request.params[2].get_str();

	if (strMasterNodePrivKey.empty()) {
		return "error privkey";
	}
	...
	int index = request.params[4].get_int();
	...
	if (nPort != nDefaultPort && !params.IsRegTestNet()) {
		return strprintf(_("Invalid -masternodeaddr port %d, only %d is supported on %s-net."),
										   nPort, nDefaultPort, Params().NetworkIDString());
	}

	CService addrTest(LookupNumeric(strHost.c_str(), nPort));
	if (!addrTest.IsValid()) {
		return strprintf(_("Invalid masternodeaddr address: %s"), strMasterNodeAddr);
	}
	...
	if (!CMessageSigner::GetKeysFromSecret(strMasterNodePrivKey, key, pubkey)) {
		return _("Invalid masternodeprivkey. Please see the documentation.");
	}

	std::string mnKeyString = EncodeSecret(key);

	COutPoint collateralOut =  COutPoint(uint256S(hash), index);
	...
	lineCopy += alias+" "+ addrTest.ToStringIPPort() +" "+mnKeyString+" "+txID+" "+indexOutStr+"\n";
	fwrite(lineCopy.c_str(), std::strlen(lineCopy.c_str()), 1, configFile);
	fclose(configFile);
	...
	masternodeConfig.add(alias, addrTest.ToStringIPPort(), mnKeyString, txID, indexOutStr);
...
UniValue initmasternode(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.empty() || request.params.size() > 2)) {
        throw std::runtime_error(
                ...
                "\nExamples:\n" +
                HelpExampleCli("initmasternode", "\"9247iC59poZmqBYt9iDh9wDam6v9S1rW5XekjLGyPnDhrDkP4AK\" \"187.24.32.124:18962\"") +
                HelpExampleRpc("initmasternode", "\"9247iC59poZmqBYt9iDh9wDam6v9S1rW5XekjLGyPnDhrDkP4AK\" \"187.24.32.124:18962\""));
    }
...
UniValue listmasternodes(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 1))
        throw std::runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode QRAX address\n"
            "    \"version\": v,        (numeric) Masternode protocol version\n"
            "    \"lastseen\": ttt,     (numeric) The time in seconds since epoch (Jan 1 1970 GMT) of the last seen\n"
            "    \"activetime\": ttt,   (numeric) The time in seconds since last seen masternode has been active\n"
...
UniValue getmasternodestatus (const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) QRAX address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners (const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) QRAX MN Address\n"
...
{ //  category              name                         actor (function)            okSafeMode
  //  --------------------- ---------------------------  --------------------------  ----------
	...
	{ "masternode",         "createmasternode",          &createmasternode,          true  },
	{ "masternode",         "deletemasternode",          &deletemasternode,          true  },
