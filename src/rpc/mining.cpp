...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    bool fPoS = consensus.NetworkUpgradeActive(nHeight + 1, Consensus::ACTIVATE_POS);
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested()) {
        ...
        // Check PoS if needed.
        if (!fPoS)
            fPoS = consensus.NetworkUpgradeActive(nHeight + 1, Consensus::ACTIVATE_POS);
    }
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
			...
			"It is set with the command line argument -gen (or qrax.conf setting gen)\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    if (fGenerate && Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::ACTIVATE_POS))
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "QRAX is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "QRAX is downloading blocks...");
...
UniValue estimatefee(const JSONRPCRequest& request)
{
   ...
   return ValueFromAmount(CAmount());
