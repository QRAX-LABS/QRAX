...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total qrax balance of the wallet\n"
            ...
            "  \"moneysupply\": n              (numeric) The transparentsupply\n"
            ...
            /*"  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in qrax/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in qrax/kb\n"*/
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"qraxaddress\"\n"
            "\nReturn information about the given qrax address.\n"

            "\nArguments:\n"
            "1. \"qraxaddress\"     (string, required) The qrax address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"qraxaddress\",    (string) The qrax address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for QRAX cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: QRAX address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are qrax addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) qrax address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"qraxaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"qraxaddress\"  (string, required) The qrax address to use for the signature.\n"
...
bool getAddressFromIndex(const int16_t &type, const uint160 &hash, std::string &address)
{
    if (type == CChainParams::Base58Type::SCRIPT_ADDRESS) {
        address = EncodeDestination(CScriptID(hash));
    } else if (type == CChainParams::Base58Type::PUBKEY_ADDRESS || type == CChainParams::Base58Type::STAKING_ADDRESS) {
        address = EncodeDestination(CKeyID(hash), (type == CChainParams::Base58Type::STAKING_ADDRESS ? true : false));
    } else {
        return false;
    }
    return true;
...
bool getIndexKey(const std::string& str, uint160& hashBytes, CChainParams::Base58Type &type)
{
    bool isStaking;
    CTxDestination dest = DecodeDestination(str, isStaking);
    ...
    type = keyID ? (isStaking ? CChainParams::Base58Type::STAKING_ADDRESS : CChainParams::Base58Type::PUBKEY_ADDRESS) : CChainParams::Base58Type::SCRIPT_ADDRESS;
    hashBytes = keyID ? *keyID : *scriptID;
    return true;
}

bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, CChainParams::Base58Type> > &addresses)
{
    uint160 hashBytes;
    CChainParams::Base58Type type = CChainParams::Base58Type::UNKNOWN_TYPE;
    if (params[0].isStr()) {
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    ...
    std::vector<std::pair<uint160, CChainParams::Base58Type> > addresses;
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<CAddressIndexKey, CAmount> >::const_iterator it=addressIndex.begin(); it!=addressIndex.end(); it++) {
        ...
        delta.pushKV("address", address);
        delta.pushKV("type", (it->first.type == CChainParams::STAKING_ADDRESS ? "stakekey" : "publickey"));
        delta.pushKV("delegation", it->first.delegated);

        if (GetAddressIdentification(it->first.hashBytes, (CChainParams::Base58Type)it->first.type, ident))
        {
            if (getAddressFromIndex(ident.type, ident.hashBytes, owner)) {
                delta.pushKV("owner", owner);
            }
        }
        result.push_back(delta);
    }
...
UniValue getaddressowner(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "getaddressowner\n"
            "\nReturns the owner for an address(es) (requires addressindex to be enabled).\n"
            "\nArguments:\n"
            "{\n"
            "  \"addresses\"\n"
            "    [\n"
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"owner\"  (string) The base58check encoded address\n"
            "  }\n"
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressowner", "'{\"addresses\": [\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\"]}'")
            + HelpExampleRpc("getaddressowner", "{\"addresses\": [\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\"]}")
        );
    ...
    for (std::vector<std::pair<uint160, CChainParams::Base58Type> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        ...
        if (GetAddressIdentification(it->first, (CChainParams::Base58Type)it->second, ident))
        {
            if (getAddressFromIndex(ident.type, ident.hashBytes, owner)) {
                UniValue delta(UniValue::VOBJ);
                delta.pushKV("address", address);
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    ...
    UniValue result(UniValue::VOBJ);
    result.pushKV("totalbalance", balance);
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "addressindex",       "getaddressutxos",        &getaddressutxos,        true },
    { "addressindex",       "getaddressdeltas",       &getaddressdeltas,       true },
    { "addressindex",       "getaddresstxids",        &getaddresstxids,        true },
    { "addressindex",       "getaddressbalance",      &getaddressbalance,      true },
    { "addressindex",       "getaddressowner",        &getaddressowner,        true },
