        ...
        bool ignoreLocked) const
{
    ...
    for (auto& p : wallet->mapWallet) {
		...
		if (!IsFinalTx(wtx.tx, wallet->GetLastBlockHeightLockWallet() + 1, GetAdjustedTime()) ||
...
libzcash::SaplingPaymentAddress SaplingScriptPubKeyMan::GenerateNewSaplingZKey()
{
    ...
    auto m_32h_cth = m_32h.Derive(1896 | ZIP32_HARDENED_KEY_LIMIT);
        ...
        int64_t nTime)
{
    ...
    auto ivk = extfvk.fvk.in_viewing_key();
    {
        ...
        if (wallet->HaveSaplingSpendingKey(extfvk)) {
            ...
            if (params.vUpgrades[Consensus::ACTIVATE_HUSH].nActivationHeight == Consensus::NetworkUpgrade::ALWAYS_ACTIVE) {
                nTimeToSet = nTime;
            } else {
