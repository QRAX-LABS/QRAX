...
isminetype IsMine(const CKeyStore& keystore, const CScript& scriptPubKey)
{
    ...
    switch (whichType) {
        ...
        case TX_DPOSASSETMINT: {
            CKeyID stakeKeyID2 = CKeyID(uint160(vSolutions[0]));
            bool stakeKeyIsMine2 = keystore.HaveKey(stakeKeyID2);
            CKeyID ownerKeyID2 = CKeyID(uint160(vSolutions[1]));
            bool spendKeyIsMine2 = keystore.HaveKey(ownerKeyID2);

            if (spendKeyIsMine2) {
                // If the wallet has both keys, ISMINE_SPENDABLE_DELEGATED
                // takes precedence over ISMINE_COLD
                return ISMINE_DPOS_ASSETS;
            } else if (stakeKeyIsMine2) {
                //return ISMINE_COLD;
            } else {
