...
const char* GetTxnOutputType(txnouttype t)
{
    ...
    case TX_ASSETSMINT: return "assetsmmint";
    case TX_DPOSASSETMINT: return "dposassetsmint";
...
static bool MatchPayToDposAssets(const CScript& script, valtype& stakerPubKeyHash, valtype& ownerPubKeyHash)
{
    if (script.IsPayAssetDposScript()) {
        stakerPubKeyHash = valtype(script.begin () + 6, script.begin() + 26);
        ownerPubKeyHash = valtype(script.begin () + 28, script.begin() + 48);
        return true;
    }
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, std::vector<std::vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (scriptPubKey.IsPayAssetScriptHash()) {
         typeRet = TX_ASSETSMINT;
         std::vector<unsigned char> hashBytes(scriptPubKey.begin()+4, scriptPubKey.begin()+24);
         vSolutionsRet.push_back(hashBytes);
         return true;
    }

    /*if (scriptPubKey.IsPayAssetDposScript()) {
         typeRet = TX_DPOSASSETMINT;
         std::vector<unsigned char> hashBytes(scriptPubKey.begin()+4, scriptPubKey.begin()+24);
         vSolutionsRet.push_back(hashBytes);
         return true;
    }*/
    ...
    if (MatchPayToDposAssets(scriptPubKey, data, data1)) {
        typeRet = TX_DPOSASSETMINT;
        vSolutionsRet.push_back(std::move(data));
        vSolutionsRet.push_back(std::move(data1));
        return true;
...
bool ExtractDestination(const CScript& scriptPubKey, CTxDestination& addressRet, bool fColdStake)
{
    ...
    } else if (whichType == TX_PUBKEYHASH || whichType == TX_ASSETSMINT) {
...
bool ExtractDestinations(const CScript& scriptPubKey, txnouttype& typeRet, std::vector<CTxDestination>& addressRet, int& nRequiredRet)
{
    ...
    if (typeRet == TX_COLDSTAKE || typeRet == TX_DPOSASSETMINT)
    {
...
CScript GetScriptForMultiminingDestination(const CKeyID& ownerKey)
{
    return CScript() << OP_DUP << OP_HASH160 << OP_ASSETSMINT <<  ToByteVector(ownerKey) << OP_EQUALVERIFY << OP_CHECKSIG;
}

CScript GetScriptForAssetsDposDestination(const CKeyID &owner, const CKeyID &staker)
{
    CScript script;
    script << OP_DUP << OP_HASH160 << OP_ROT <<
            OP_IF << OP_DPOSASSETSMINT << ToByteVector(staker) <<
            OP_ELSE << ToByteVector(owner) << OP_ENDIF <<
            OP_EQUALVERIFY << OP_CHECKSIG;
    return script;
}
