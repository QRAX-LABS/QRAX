...
#ifndef QRAX_SERIALIZE_H
#define QRAX_SERIALIZE_H
...
#include "prevector.h"
#include "sporkid.h"
#include "multimining/multimining_program_ids.h"
...
// Serialization for multiminingProgramId
template <typename Stream>
inline void Serialize(Stream& s, multiminingProgramId programID)
{
    int32_t f = static_cast<int32_t>(programID);
    ser_writedata32(s, f);
}

template <typename Stream>
inline void Unserialize(Stream& s, multiminingProgramId& programID)
{
    int32_t f = ser_readdata32(s);
    programID = (multiminingProgramId) f;
}
