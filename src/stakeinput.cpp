...
CNewStake* CNewStake::NewStake(const CTxIn& txin)
{
	...
	return new CNewStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CNewStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CNewStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CNewStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CNewStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
    ...
    if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH && whichType != TX_COLDSTAKE && whichType != TX_ASSETSMINT && whichType != TX_DPOSASSETMINT)
        return error("%s: type=%d (%s) not supported for scriptPubKeyKernel", __func__, whichType, GetTxnOutputType(whichType));

    CScript scriptPubKey;
    CKey key;
    if (whichType == TX_PUBKEYHASH || whichType == TX_COLDSTAKE  || whichType == TX_ASSETSMINT || whichType == TX_DPOSASSETMINT) {
    ...
    if (onlyP2PK && (whichType == TX_PUBKEYHASH  || whichType == TX_ASSETSMINT)) {
...
CDataStream CNewStake::GetUniqueness() const
{
    //The unique identifier for a QRAX stake is the outpoint
...
const CBlockIndex* CNewStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
	if (!pindexFrom) throw std::runtime_error("CNewStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CNewStake::ContextCheck(int nHeight, uint32_t nTime)
{
