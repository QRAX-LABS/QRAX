...
BOOST_AUTO_TEST_CASE(budget_value)
{
    ...
    /*int nHeightTest = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight + 1;
    CheckBudgetValue(nHeightTest-1, "testnet", 7200*COIN);
    CheckBudgetValue(nHeightTest, "testnet", 144*COIN);

    SelectParams(CBaseChainParams::MAIN);
    nHeightTest = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight + 1;
