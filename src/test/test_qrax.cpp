...
#define BOOST_TEST_MODULE Qrax Test Suite
...
{
        ClearDatadirCache();
	pathTemp = GetTempPath() / strprintf("test_qrax_%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(100000)));
...
TestChainSetup::TestChainSetup(int blockCount)
{
    ...
    if (blockCount > Params().GetConsensus().vUpgrades[Consensus::ACTIVATE_POS].nActivationHeight) {
        UpdateNetworkUpgradeParameters(Consensus::ACTIVATE_POS, blockCount + 100);
    }
