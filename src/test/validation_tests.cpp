...
void test_simple_sapling_invalidity(CMutableTransaction& tx)
{
    ...
    }
    {
		...
		BOOST_CHECK(!CheckTransaction(newTx, state, false, true));
...
BOOST_AUTO_TEST_CASE(test_simple_shielded_invalid)
{
    ...
    UpdateNetworkUpgradeParameters(Consensus::ACTIVATE_HUSH, Consensus::NetworkUpgrade::ALWAYS_ACTIVE);
    test_simple_sapling_invalidity(mtx);
    UpdateNetworkUpgradeParameters(Consensus::ACTIVATE_HUSH, Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT);
