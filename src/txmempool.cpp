...
void CTxMemPool::addAddressIndex(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (prevout.scriptPubKey.IsPayToScriptHash()) {
            ...
            CMempoolAddressDeltaKey key(CChainParams::Base58Type::SCRIPT_ADDRESS, uint160(hashBytes), txhash, j, 1);
            CMempoolAddressDelta delta(entry.GetTime(), prevout.nValue * -1, input.prevout.hash, input.prevout.n);
            mapAddress.insert(std::make_pair(key, delta));
            inserted.push_back(key);
        } else if (prevout.scriptPubKey.IsPayToPublicKeyHash()) {
            std::vector<unsigned char> hashBytes(prevout.scriptPubKey.begin()+3, prevout.scriptPubKey.begin()+23);
            CMempoolAddressDeltaKey key(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytes), txhash, j, 1);
            CMempoolAddressDelta delta(entry.GetTime(), prevout.nValue * -1, input.prevout.hash, input.prevout.n);
            mapAddress.insert(std::make_pair(key, delta));
            inserted.push_back(key);
        } else if (prevout.scriptPubKey.IsPayToPublicKey()) {
            uint160 hashBytes(Hash160(prevout.scriptPubKey.begin()+1, prevout.scriptPubKey.end()-1));
            CMempoolAddressDeltaKey key(CChainParams::Base58Type::PUBKEY_ADDRESS, hashBytes, txhash, j, 1);
            CMempoolAddressDelta delta(entry.GetTime(), prevout.nValue * -1, input.prevout.hash, input.prevout.n);
            mapAddress.insert(std::make_pair(key, delta));
            inserted.push_back(key);
        } else if (prevout.scriptPubKey.IsPayAssetScriptHash()) {
            std::vector<unsigned char> hashBytes(prevout.scriptPubKey.begin()+4, prevout.scriptPubKey.begin()+24);
            CMempoolAddressDeltaKey key(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytes), txhash, j, 1);
            CMempoolAddressDelta delta(entry.GetTime(), prevout.nValue * -1, input.prevout.hash, input.prevout.n);
            mapAddress.insert(std::make_pair(key, delta));
            inserted.push_back(key);
        } else if (prevout.scriptPubKey.IsPayToColdStaking() || prevout.scriptPubKey.IsPayAssetDposScript())
        {
            std::vector<unsigned char> hashBytesStaker(prevout.scriptPubKey.begin()+6, prevout.scriptPubKey.begin()+26);
            CMempoolAddressDeltaKey keyS(CChainParams::Base58Type::STAKING_ADDRESS, uint160(hashBytesStaker), txhash, j, 1);
            CMempoolAddressDelta deltaS(entry.GetTime(), prevout.nValue * -1, input.prevout.hash, input.prevout.n);
            mapAddress.insert(std::make_pair(keyS, deltaS));
            std::vector<unsigned char> hashBytesOwner(prevout.scriptPubKey.begin()+28, prevout.scriptPubKey.begin()+48);
            CMempoolAddressDeltaKey keyO(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytesOwner), txhash, j, 1);
            CMempoolAddressDelta deltaO(entry.GetTime(), prevout.nValue * -1, input.prevout.hash, input.prevout.n);
            mapAddress.insert(std::make_pair(keyO, deltaO));
        }
    }
    ...
    for (unsigned int k = 0; k < tx.vout.size(); k++) {
        ...
        if (out.scriptPubKey.IsPayToScriptHash()) {
            ...
            CMempoolAddressDeltaKey key(CChainParams::Base58Type::SCRIPT_ADDRESS, uint160(hashBytes), txhash, k, 0);
            mapAddress.insert(std::make_pair(key, CMempoolAddressDelta(entry.GetTime(), out.nValue)));
            inserted.push_back(key);
        } else if (out.scriptPubKey.IsPayToPublicKeyHash()) {
            std::vector<unsigned char> hashBytes(out.scriptPubKey.begin()+3, out.scriptPubKey.begin()+23);
            std::pair<addressDeltaMap::iterator,bool> ret;
            CMempoolAddressDeltaKey key(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytes), txhash, k, 0);
            mapAddress.insert(std::make_pair(key, CMempoolAddressDelta(entry.GetTime(), out.nValue)));
            inserted.push_back(key);
        } else if (out.scriptPubKey.IsPayToPublicKey()) {
            uint160 hashBytes(Hash160(out.scriptPubKey.begin()+1, out.scriptPubKey.end()-1));
            std::pair<addressDeltaMap::iterator,bool> ret;
            CMempoolAddressDeltaKey key(CChainParams::Base58Type::PUBKEY_ADDRESS, hashBytes, txhash, k, 0);
            mapAddress.insert(std::make_pair(key, CMempoolAddressDelta(entry.GetTime(), out.nValue)));
            inserted.push_back(key);
            ...
            mapAddress.insert(std::make_pair(key, CMempoolAddressDelta(entry.GetTime(), out.nValue)));
            inserted.push_back(key);
        } else if (out.scriptPubKey.IsPayToColdStaking() || out.scriptPubKey.IsPayAssetDposScript()) {
            std::vector<unsigned char> hashBytesStaker(out.scriptPubKey.begin()+6, out.scriptPubKey.begin()+26);
            CMempoolAddressDeltaKey keyS(CChainParams::Base58Type::STAKING_ADDRESS, uint160(hashBytesStaker), txhash, k, 0);
            mapAddress.insert(std::make_pair(keyS, CMempoolAddressDelta(entry.GetTime(), out.nValue)));
            inserted.push_back(keyS);

            std::vector<unsigned char> hashBytesOwner(out.scriptPubKey.begin()+28, out.scriptPubKey.begin()+48);
            CMempoolAddressDeltaKey keyO(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytesOwner), txhash, k, 0);
            mapAddress.insert(std::make_pair(keyO, CMempoolAddressDelta(entry.GetTime(), out.nValue)));
            inserted.push_back(keyO);
        }
    }
...
bool CTxMemPool::getAddressIndex(std::vector<std::pair<uint160, CChainParams::Base58Type> > &addresses,
                                 std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> > &results)
{
    LOCK(cs);
    for (std::vector<std::pair<uint160, CChainParams::Base58Type> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
...
void CTxMemPool::addSpentIndex(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        const Coin& coin = view.AccessCoin(input.prevout);
        const CTxOut &prevout = coin.out;
        uint160 addressHash, addressHashStaker;
        ...
        if (prevout.scriptPubKey.IsPayToScriptHash()) {
            ...
            addressHash = uint160(std::vector<unsigned char> (prevout.scriptPubKey.begin()+4, prevout.scriptPubKey.begin()+24));
            addressType = CChainParams::Base58Type::PUBKEY_ADDRESS;
        } else if (prevout.scriptPubKey.IsPayToColdStaking() || prevout.scriptPubKey.IsPayAssetDposScript()) {
            addressHashStaker = uint160(std::vector<unsigned char> (prevout.scriptPubKey.begin()+6, prevout.scriptPubKey.begin()+26));
            addressTypeStaker = CChainParams::Base58Type::STAKING_ADDRESS;
            addressHash = uint160(std::vector<unsigned char> (prevout.scriptPubKey.begin()+28, prevout.scriptPubKey.begin()+48));
            addressType = CChainParams::Base58Type::PUBKEY_ADDRESS;
            isColdStake = true;
        } else {
            addressHash.SetNull();
            addressHashStaker.SetNull();
        ...
        if (isColdStake) {
            CSpentIndexKey keyS = CSpentIndexKey(input.prevout.hash, input.prevout.n);
            CSpentIndexValue valueS = CSpentIndexValue(txhash, j, -1, prevout.nValue, addressTypeStaker, addressHashStaker);
            mapSpent.insert(std::make_pair(keyS, valueS));
            inserted.push_back(keyS);
        }
                                ...
                                bool fCurrentEstimate)
{
    ...
    lastRollingFeeUpdate = GetAdjustedTime();
