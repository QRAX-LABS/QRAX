...
const char * const QRAX_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\QRAX
// Windows >= Vista: C:\Users\Username\AppData\Roaming\QRAX
// Mac: ~/Library/Application Support/QRAX
// Unix: ~/.qrax
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QRAX";
#else
    fs::path pathRet;
...
static fs::path GetBaseParamsDir()
{
    // Copied from GetDefaultDataDir and adapter for zcash params.
	// Windows < Vista: C:\Documents and Settings\Username\Application Data\QRAXParams
	// Windows >= Vista: C:\Users\Username\AppData\Roaming\QRAXParams
	// Mac: ~/Library/Application Support/QRAXParams
    // Unix: ~/.qrax-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QRAXParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "QRAXParams";
#else
    // Unix
    return pathRet / ".qrax-params";
#endif
#endif
}
...
void initZKSNARKS()
{
	const fs::path& path = GetParamsDir();
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/qrax/sapling-spend.params";
        sapling_output = "/usr/share/qrax/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/qrax/sapling-spend.params";
            sapling_output = "/usr/local/share/qrax/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", QRAX_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", QRAX_PID_FILENAME));
