...
#include "multimining/multimining_db.h"
#include "multimining/multimining_programs.h"
#include "multimining/multimining_program_db.h"
...
#if defined(NDEBUG)
#error "QRAX cannot be compiled without assertions."
#endif
...
std::atomic<bool> fMultiMiningIsInit{false};
//extern std::atomic<bool> fMultiWalletsNode;
 ...
 */
CFeeRate minRelayTxFee = CFeeRate(500000);
...
CMultiMiningDB* multiMiningDB = NULL;
CMultiMiningProgramDB* mmProgramDB = NULL;
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        bool hasMmSpendInuts = tx.ContainsAssets();
...
bool GetAddressIndex(uint160 addressHash, CChainParams::Base58Type type,
...
bool GetSupplyIndexByHeight(CAmount &amount, int nHeight)
{
    if (!fAddressIndex)
        return error("address index not enabled");

    if (!pblocktree->ReadSupplyIndexByHeight(amount, nHeight))
        return error("unable to get txids for address");

     return true;
}

bool GetAddressUnspent(uint160 addressHash, CChainParams::Base58Type type,
...
bool GetAddressIdentification(uint160 addressHash, CChainParams::Base58Type type, CAddressOwnerIdentificator &addressIdent)
{
    if (!fAddressIndex)
        return error("address index not enabled");
    if (!pblocktree->ReadAdressIdentification(addressHash, type, addressIdent)) return false;
    return true;
}

bool GetAddressListByIdentification(uint160 addressHash, CChainParams::Base58Type type, std::vector<CAddressIndexIteratorKey> &addressIdent)
{
    if (!fAddressIndex)
        return error("address index not enabled");
    if (!pblocktree->ReadAddressesByIdentificator(addressHash, type, addressIdent)) return false;
    return true;
}
...
bool GetTransactionSenderWalletId(const uint256 &hash, unsigned int n, CKeyID &outKey)
{
    /*CTransactionRef tx;
    ...
    CTransactionRef txPrev;
    uint256 hashBlockPrev = uint256();
    if (GetTransaction(hash, txPrev, hashBlockPrev, true))
    {
        txnouttype type;
        std::vector<CTxDestination> addresses;
        int nRequired;
        if (ExtractDestinations(txPrev->vout[n].scriptPubKey, type, addresses, nRequired)) {
            CKeyID *keyID = boost::get<CKeyID>(&addresses[0]);
            if (keyID != nullptr && !keyID->IsNull())
            {
                CAddressOwnerIdentificator owner;
                if (GetAddressIdentification(*keyID, CChainParams::PUBKEY_ADDRESS, owner)) {
                    outKey = CKeyID(owner.hashBytes);
                    return true;
                }
            }
        }
...
CAmount GetBlockValue(int nHeight)
{
    ...
    const Consensus::Params& consensus = Params().GetConsensus();
    const bool isPoSActive = consensus.NetworkUpgradeActive(nHeight, Consensus::ACTIVATE_POS);
    ...
    {
        return 1200400 * COIN;
    } else if (nHeight == 1 && Params().NetworkIDString() == CBaseChainParams::TESTNET)
        return 12004000 * COIN;

    if (nHeight > 1 && !isPoSActive)
    {
        if (nHeight >= 30000 && nHeight < 35000)
            return 2200  * COIN;

        return 200 * COIN;
    }

    const int period = consensus.nBudgetCycleBlocks;
    const int count = (nHeight / period) - 1;

    if (count <= 0) return 10 * COIN;
    if (count >= 10) return 1 * COIN;

    return (10 - count) * COIN;

}

int64_t GetMasternodePayment(int nHeight)
{
    const int period = Params().GetConsensus().nBudgetCycleBlocks;
    int count = (nHeight / period) - 1;

    if (count  <= 0) return 50 * COIN;
    if (count >= 10) return 5 * COIN;

    return (50 - (count * 5)) * COIN;
}
...
static bool SkipInvalidUTXOS(int nHeight)
{
    ...
    return Params().NetworkIDString() == CBaseChainParams::MAIN &&
           consensus.NetworkUpgradeActive(nHeight, Consensus::ACTIVATE_POS);
}
...
DisconnectResult DisconnectBlock(CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
	...
	std::vector<std::pair<CAddressDelegationIndexKey, CAmount> > addressDelegationIndex;
	std::vector<std::pair<CAddressDelegationUnspentKey, CAddressUnspentValue> > addressDelegationUnspentIndex;

    CAmount nValueIn = 0;
    CAmount nValueOut = 0;
    CAmount nAssetValue = 0;
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (fAddressIndex) {
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
				...
				bool isColdStaking = out.scriptPubKey.IsPayToColdStaking();

                std::vector<uint160> list = GetHashBytesFromScriptPubKey(out.scriptPubKey);
				if (list.size() == 1) {
					CChainParams::Base58Type type;
					if (out.scriptPubKey.IsPayToScriptHash()) {
						type = CChainParams::Base58Type::SCRIPT_ADDRESS;
					} else if (out.scriptPubKey.IsPayToPublicKeyHash() || out.scriptPubKey.IsPayToPublicKey() || out.scriptPubKey.IsPayAssetScriptHash()) {
						type = CChainParams::Base58Type::PUBKEY_ADDRESS;
					}
					addressIndex.push_back(std::make_pair(CAddressIndexKey(type, list[0], pindex->nHeight, i, hash, k, false, false), out.nValue * -1));
					addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(type, list[0], hash, k), CAddressUnspentValue()));
				}

				if (list.size() == 2) {

					if (isColdStaking) {
						addressDelegationIndex.push_back(std::make_pair(CAddressDelegationIndexKey(list[0], list[1], pindex->nHeight, i, hash, k, false), out.nValue * -1));
						addressDelegationUnspentIndex.push_back(std::make_pair(CAddressDelegationUnspentKey(list[0], list[1], hash, k), CAddressUnspentValue()));
					}
					addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::PUBKEY_ADDRESS, list[1], pindex->nHeight, i, hash, k, false, isColdStaking), out.nValue * -1));
					addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(CChainParams::Base58Type::PUBKEY_ADDRESS, list[1], hash, k), CAddressUnspentValue()));
				}
        ...
        if (tx.ContainsAssets() && !fVerifyingBlocks) {
        ...
        for (unsigned int j = tx.vin.size(); j-- > 0;) {
            ...
            if (fAddressIndex) {
                ...
                std::vector<uint160> list = GetHashBytesFromScriptPubKey(prevout.scriptPubKey);
				bool isColdStake = prevout.scriptPubKey.IsPayToColdStaking();

				if (list.size() == 1) {
					CChainParams::Base58Type type;
					if (prevout.scriptPubKey.IsPayToScriptHash()) {
						type = CChainParams::Base58Type::SCRIPT_ADDRESS;
					} else if (prevout.scriptPubKey.IsPayToPublicKeyHash() || prevout.scriptPubKey.IsPayToPublicKey() || prevout.scriptPubKey.IsPayAssetScriptHash()) {
						type = CChainParams::Base58Type::PUBKEY_ADDRESS;
					}
					addressIndex.push_back(std::make_pair(CAddressIndexKey(type, list[0], pindex->nHeight, i, hash, j, true, false), prevout.nValue));
					addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(type, list[0], input.prevout.hash, input.prevout.n), CAddressUnspentValue(prevout.nValue, prevout.scriptPubKey, undoHeight)));
				}
				if (list.size() == 2) {

					if (isColdStake) {
						addressDelegationIndex.push_back(std::make_pair(CAddressDelegationIndexKey(list[0], list[1], pindex->nHeight, i, hash, j, true), prevout.nValue));
						addressDelegationUnspentIndex.push_back(std::make_pair(CAddressDelegationUnspentKey(list[0], list[1], input.prevout.hash, input.prevout.n), CAddressUnspentValue(prevout.nValue, prevout.scriptPubKey, undoHeight)));
					}
    ...
    for(auto& reg : block.vReg)
    {
        if (reg.owner.IsNull() || reg.key.IsNull()) continue;
        removedIdentsKeys.emplace_back(CAddressIndexIteratorKey(reg.type, reg.key));
    }
    ...
    if (fAddressIndex) {
		...
		if (!pblocktree->EraseAddressDelegationIndex(addressDelegationIndex))
		{
			AbortNode("Failed to delete address delegation index");
			return DISCONNECT_FAILED;
		}
        multiMiningTree.SetAddressIndexList(addressIndex, false);
        if (!pblocktree->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            AbortNode("Failed to write address unspent index");
            return DISCONNECT_FAILED;
        }

		if (!pblocktree->UpdateAddressDelegationUnspentIndex(addressDelegationUnspentIndex)) {
			AbortNode("Failed to write address delegation unspent index");
			return DISCONNECT_FAILED;
		}

        if (!pblocktree->EraseAddressIdentification(removedIdentsKeys)) {
    ...
    // empty root.
    if (consensus.NetworkUpgradeActive(pindex->pprev->nHeight, Consensus::ACTIVATE_HUSH)) {
...
std::vector<uint160> GetHashBytesFromScriptPubKey(CScript script)
{
    std::vector<uint160> set;
    if (script.IsPayToScriptHash()) {
        std::vector<unsigned char> hashBytes(script.begin()+2, script.begin()+22);
         set.push_back(uint160(hashBytes));
       ...
       set.push_back(uint160(hashBytes));
        ...
        set.push_back(hashBytes);
    }  else if (script.IsPayAssetScriptHash())  {
        std::vector<unsigned char> hashBytes(script.begin()+4, script.begin()+24);
        set.push_back(uint160(hashBytes));
    } else if (script.IsPayToColdStaking() || script.IsPayAssetDposScript()) {
        std::vector<unsigned char> hashBytesStaker(script.begin()+6, script.begin()+26);
        std::vector<unsigned char> hashBytesOwner(script.begin()+28, script.begin()+48);
        set.push_back(uint160(hashBytesStaker));
        set.push_back(uint160(hashBytesOwner));
    }
    return set;
}
...
{
    util::ThreadRename("qrax-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    const bool isPoSActive = consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::ACTIVATE_POS);
    ...
    std::vector<CKeyID> vSpendsInBlock;
    ...
    //
    bool isV5UpgradeEnforced = consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::ACTIVATE_HUSH);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (!tx.IsCoinBase() && !tx.ContainsAssets()) {
            ...
            if (fAddressIndex || fSpentIndex)
            {
                for (size_t j = 0; j < tx.vin.size(); j++) {
                    ...
                    const Coin& coin = view.AccessCoin(tx.vin[j].prevout);
                    const CTxOut &prevout = coin.out;
                    uint160 hashBytes, hashBytesStaker;
                    CChainParams::Base58Type addressType, addressTypeStaker;
                    bool isColdstake = false;
                    bool isDposReward = false;
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
                        hashBytes = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin()+2, prevout.scriptPubKey.begin()+22));
                        addressType = CChainParams::Base58Type::SCRIPT_ADDRESS;
                    } else if (prevout.scriptPubKey.IsPayToPublicKeyHash()) {
                        hashBytes = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin()+3, prevout.scriptPubKey.begin()+23));
                        addressType = CChainParams::Base58Type::PUBKEY_ADDRESS;
                    } else if (prevout.scriptPubKey.IsPayToPublicKey()) {
                        hashBytes = Hash160(prevout.scriptPubKey.begin()+1, prevout.scriptPubKey.end()-1);
                        addressType = CChainParams::Base58Type::PUBKEY_ADDRESS;
                    } else if (prevout.scriptPubKey.IsPayAssetScriptHash()) {
                        hashBytes = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin()+4, prevout.scriptPubKey.begin()+24));
                        addressType = CChainParams::Base58Type::PUBKEY_ADDRESS;
					} else if (prevout.scriptPubKey.IsPayToColdStaking() ) {
                        isColdstake = true;
                        hashBytes = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin()+28, prevout.scriptPubKey.begin()+48));
                        hashBytesStaker = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin()+6, prevout.scriptPubKey.begin()+26));
                        addressType = CChainParams::Base58Type::PUBKEY_ADDRESS;
                        addressTypeStaker = CChainParams::Base58Type::STAKING_ADDRESS;
					} else if (prevout.scriptPubKey.IsPayAssetDposScript()) {
						isDposReward = true;
						hashBytes = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin()+28, prevout.scriptPubKey.begin()+48));
						hashBytesStaker = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin()+6, prevout.scriptPubKey.begin()+26));
						addressType = CChainParams::Base58Type::PUBKEY_ADDRESS;
						addressTypeStaker = CChainParams::Base58Type::PUBKEY_ADDRESS;
					} else {
                        hashBytes.SetNull();
                        hashBytesStaker.SetNull();
                        addressType = CChainParams::Base58Type::UNKNOWN_TYPE;
                        addressTypeStaker = CChainParams::Base58Type::UNKNOWN_TYPE;
                    }

                    if (fAddressIndex && addressType > CChainParams::Base58Type::UNKNOWN_TYPE) {
                        // record spending activity
						addressIndex.push_back(std::make_pair(CAddressIndexKey(addressType, hashBytes, pindex->nHeight, i, txhash, j, true, isColdstake), prevout.nValue * -1));
						//if (isColdstake && addressTypeStaker > CChainParams::Base58Type::UNKNOWN_TYPE) addressIndex.push_back(std::make_pair(CAddressIndexKey(addressTypeStaker, hashBytesStaker, pindex->nHeight, i, txhash, j, true, true), prevout.nValue * -1));
						//if (isDposReward && addressTypeStaker > CChainParams::Base58Type::UNKNOWN_TYPE) addressIndex.push_back(std::make_pair(CAddressIndexKey(addressTypeStaker, hashBytesStaker, pindex->nHeight, i, txhash, j, true, false), prevout.nValue * -1));
						...
						//if (isColdstake && addressTypeStaker > CChainParams::Base58Type::UNKNOWN_TYPE) addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(addressTypeStaker, hashBytesStaker, input.prevout.hash, input.prevout.n), CAddressUnspentValue()));
						//if (isDposReward && addressTypeStaker > CChainParams::Base58Type::UNKNOWN_TYPE) addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(addressTypeStaker, hashBytesStaker, input.prevout.hash, input.prevout.n), CAddressUnspentValue()));
						if (isColdstake) {
							addressDelegationIndex.push_back(std::make_pair(CAddressDelegationIndexKey(hashBytesStaker, hashBytes, pindex->nHeight, i, txhash, j, true), prevout.nValue * -1));
							addressDelegationUnspentIndex.push_back(std::make_pair(CAddressDelegationUnspentKey(hashBytesStaker, hashBytes, input.prevout.hash, input.prevout.n), CAddressUnspentValue()));
						}
                    }

                    if (fSpentIndex && addressType > CChainParams::Base58Type::UNKNOWN_TYPE) {
						...
						if ((isColdstake || isDposReward) && addressTypeStaker > CChainParams::Base58Type::UNKNOWN_TYPE) spentIndex.push_back(std::make_pair(CSpentIndexKey(input.prevout.hash, input.prevout.n), CSpentIndexValue(txhash, j, pindex->nHeight, prevout.nValue, addressTypeStaker, hashBytesStaker)));
                    }
        ...
        if (fAddressIndex) {
            ...
            for (unsigned int k = 0; k < tx.vout.size(); k++) {
                ...
                const COutPoint outPoint(txhash, k);
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
                    ...
                    addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::SCRIPT_ADDRESS, uint160(hashBytes), pindex->nHeight, i, txhash, k, false, false), out.nValue));
                    // record unspent output
                    addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(CChainParams::Base58Type::SCRIPT_ADDRESS, uint160(hashBytes), txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->nHeight)));

                } else if (out.scriptPubKey.IsPayToPublicKeyHash()) {
                    std::vector<unsigned char> hashBytes(out.scriptPubKey.begin()+3, out.scriptPubKey.begin()+23);
                    addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytes), pindex->nHeight, i, txhash, k, false, false), out.nValue));
                    addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytes), txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->nHeight)));
                } else if (out.scriptPubKey.IsPayToPublicKey()) {
                    uint160 hashBytes(Hash160(out.scriptPubKey.begin()+1, out.scriptPubKey.end()-1));
                    addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::PUBKEY_ADDRESS, hashBytes, pindex->nHeight, i, txhash, k, false, false), out.nValue));
                    addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(CChainParams::Base58Type::PUBKEY_ADDRESS, hashBytes, txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->nHeight)));
                } else if (out.scriptPubKey.IsPayAssetScriptHash()) {
                    std::vector<unsigned char> hashBytes(out.scriptPubKey.begin()+4, out.scriptPubKey.begin()+24);
                    addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytes), pindex->nHeight, i, txhash, k, false, false), out.nValue));
                    addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytes), txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->nHeight)));
				} else if (out.scriptPubKey.IsPayToColdStaking()) {
                    std::vector<unsigned char> hashBytesStake(out.scriptPubKey.begin()+6, out.scriptPubKey.begin()+26);
                    std::vector<unsigned char> hashBytesOwner(out.scriptPubKey.begin()+28, out.scriptPubKey.begin()+48);
					//addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::STAKING_ADDRESS, uint160(hashBytesStake), pindex->nHeight, i, txhash, k, false, true), out.nValue));
					//addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(CChainParams::Base58Type::STAKING_ADDRESS, uint160(hashBytesStake), txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->nHeight)));
					addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytesOwner), pindex->nHeight, i, txhash, k, false, true), out.nValue));
					...
					addressDelegationIndex.push_back(std::make_pair(CAddressDelegationIndexKey(uint160(hashBytesStake), uint160(hashBytesOwner), pindex->nHeight, i, txhash, k, false), out.nValue));
					addressDelegationUnspentIndex.push_back(std::make_pair(CAddressDelegationUnspentKey(uint160(hashBytesStake), uint160(hashBytesOwner), txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->nHeight)));

				} else if ( out.scriptPubKey.IsPayAssetDposScript()) {
					//std::vector<unsigned char> hashBytesStake(out.scriptPubKey.begin()+6, out.scriptPubKey.begin()+26);
					std::vector<unsigned char> hashBytesOwner(out.scriptPubKey.begin()+28, out.scriptPubKey.begin()+48);
					//addressIndex.push_back(std::make_pair(CAddressIndexKey(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytesStake), pindex->nHeight, i, txhash, k, false, false), out.nValue));
					//addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(CChainParams::Base58Type::PUBKEY_ADDRESS, uint160(hashBytesStake), txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->nHeight)));
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::ACTIVATE_HUSH)) {
	...
	std::vector<std::pair<CAddressIndexIteratorKey, CAddressOwnerIdentificator > > identsKeys;
	for(auto& reg : block.vReg)
	{
		if (reg.owner.IsNull() || reg.key.IsNull()) continue;
		std::pair<CAddressIndexIteratorKey, CAddressOwnerIdentificator> pair = std::make_pair(CAddressIndexIteratorKey(reg.type, reg.key), CAddressOwnerIdentificator(CChainParams::Base58Type::PUBKEY_ADDRESS, reg.owner, pindex->nHeight));
		identsKeys.emplace_back(pair);
	}

    // track mint amount info
	CAmount AssetToMint = 0;
	if (!consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::ACTIVATE_ASSET_VALIDATION)) {
		AssetToMint = -nAssetValue;
	}

	const int64_t nMint = (nValueOut - nValueIn) + nFees + AssetToMint;

    int64_t nTime1 = GetTimeMicros();
    nTimeConnect += nTime1 - nTimeStart;
    LogPrint(BCLog::BENCH, "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n", (unsigned)block.vtx.size(), 0.001 * (nTime1 - nTimeStart), 0.001 * (nTime1 - nTimeStart) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime1 - nTimeStart) / (nInputs - 1), nTimeConnect * 0.000001);

	CAmount mnPay = GetMasternodePayment(pindex->nHeight);

    //PoW phase redistributed fees to miner. PoS stage destroys fees.
	CAmount nExpectedMint = GetBlockValue(pindex->nHeight) + nFees + mnPay;

	if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::ACTIVATE_ASSET_VALIDATION)) {
		nExpectedMint += multiMiningManager.GetSummaryAssetsToReward();
	}
    ...
    if (fAddressIndex) {
        ...
        if (!pblocktree->WriteAddressIdentification(identsKeys)) {
            return AbortNode(state, "Failed to write address identification index");
        }

		if (!pblocktree->WriteDelegationAddressIndex(addressDelegationIndex)) {
			return AbortNode(state, "Failed to write address delegation index");
		}

		if (!pblocktree->UpdateAddressDelegationUnspentIndex(addressDelegationUnspentIndex)) {
			return AbortNode(state, "Failed to write address delegation unspent index");
		}
    ...
    for (const auto& reg : block.vReg)
    {
        multiMiningManager.RemoveRegistration(reg);
    }
    ...
    try {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                if (!multiMiningDB->WriteBatchSync(true))
                {
                    return AbortNode(state, "Filed to write to multimining database");
                }
            }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	g_newP2CSRules = Params().GetConsensus().NetworkUpgradeActive(pindexNew->nHeight, Consensus::ACTIVATE_NEW_COLDSTAKE);
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if (!consensus.NetworkUpgradeActive(pindexNew->nHeight, Consensus::ACTIVATE_POS)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	const int blockHeight = chainActive.Height() + 1;
	const Consensus::Params& consensus = Params().GetConsensus();

	const bool assetValidation = consensus.NetworkUpgradeActive(blockHeight, Consensus::ACTIVATE_ASSET_VALIDATION);
	if (assetValidation && block.assetRootHash != multiMiningManager.GetCurrentAssetHash())
		return state.DoS(100, false, REJECT_INVALID, "bad-asset-hash-tree", true, "invalid asset tree hash");
    ...
    for (unsigned int i = 1; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinBase() && !block.vtx[i]->ContainsAssets())
    ...
    bool fSaplingActive = consensus.NetworkUpgradeActive(blockHeight, Consensus::ACTIVATE_HUSH);
...
bool static LoadBlockIndexDB(std::string& strError)
{
    ...
    pblocktree->ReadFlag("multiwalletsnode", fMultiWalletsNode);
    LogPrintf("%s: MultiWalletsNode %s\n", __func__, fMultiWalletsNode ? "enabled" : "disabled");
...
bool LoadChainTip(const CChainParams& chainparams)
{
	...
	g_newP2CSRules = Params().GetConsensus().NetworkUpgradeActive(pChainTip->nHeight, Consensus::ACTIVATE_NEW_COLDSTAKE);
...
bool LoadBlockIndex(std::string& strError)
{
    ...
    if (!fReindex) {
        bool ret = LoadBlockIndexDB(strError);
        if (!ret) return false;
    ...
    if (needs_init) {
        ...
        fMultiWalletsNode = gArgs.GetBoolArg("-multiwallets", DEFAULT_MULTIWALLETS);
        pblocktree->WriteFlag("multiwalletsnode", fMultiWalletsNode);
