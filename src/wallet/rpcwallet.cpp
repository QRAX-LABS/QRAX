...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    entry.pushKV("walletSenderId", wtx.GetWalletId().GetHex());
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
	...
	const std::string example_address = "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\"";

    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                "getaddressinfo ( \"address\" )\n"
                "\nReturn information about the given QRAX address.\n"
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new QRAX address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"qraxaddress\"    (string) The new qrax address\n"
...
UniValue getnewstakingaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new QRAX cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
			HelpExampleCli("delegatoradd", "QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc") +
			HelpExampleRpc("delegatoradd", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\"") +
			HelpExampleRpc("delegatoradd", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" \"myPaperWallet\""));
    ...
    if (!IsValidDestination(dest) || isStakingAddress)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid QRAX address");

    const std::string strLabel = (request.params.size() > 1 ? request.params[1].get_str() : "");

    const CKeyID* keyID = boost::get<CKeyID>(&dest);
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from QRAX address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
			HelpExampleCli("delegatorremove", "QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc") +
			HelpExampleRpc("delegatorremove", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\""));
    ...
    if (!pwalletMain->HasAddressBook(*keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get QRAX address from addressBook");
...
UniValue listdelegators(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) QRAX address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new QRAX address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"qraxaddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"qraxaddress\"   (string, required) The qrax address to be associated with a label.\n"
            "2. \"label\"         (string, required) The label to assign to the address.\n"

            "\nExamples:\n" +
			HelpExampleCli("setlabel", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" \"tabby\"") + HelpExampleRpc("setlabel", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\", \"tabby\""));
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"qraxaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"qraxaddress\"  (string, required) The qrax address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in QRAX to send. eg 0.1\n"
            ...
            "\nExamples:\n" +
			HelpExampleCli("sendtoaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" 0.1") +
			HelpExampleCli("sendtoaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" 0.1 \"donation\" \"seans outpost\"") +
			HelpExampleRpc("sendtoaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\", 0.1, \"donation\", \"seans outpost\""));
...
UniValue CreateColdStakeDelegation(const UniValue& params, CTransactionRef& txNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || !isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid QRAX staking address");
    ...
    if (!fUseShielded) {
        ...
        if (!consensus.NetworkUpgradeActive(nextBlockHeight, Consensus::ACTIVATE_HUSH)) {
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "2. \"amount\"              (numeric, required) The amount in QRAX to delegate for staking. eg 100\n"
...
UniValue rawdelegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
			...
			HelpExampleCli("rawdelegatestake", "\"S1t2a3kab9c8c71VA78xxxy4MxZg6vgeS6\" 1000 \"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\"") +
			HelpExampleRpc("rawdelegatestake", "\"S1t2a3kab9c8c71VA78xxxy4MxZg6vgeS6\", 1000, \"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\""));
...
UniValue viewshieldtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                ...
                "  \"spends\" : [\n"
                "    {\n"
				"      \"spend\" : n,                    (numeric, hush) the index of the spend within vShieldedSpend\n"
                "      \"txidPrev\" : \"transactionid\",   (string) The id for the transaction this note was created in\n"
				"      \"outputPrev\" : n,               (numeric, hush) the index of the output within the vShieldedOutput\n"
                "      \"address\" : \"qraxaddress\",     (string) The QRAX address involved in the transaction\n"
                ...
                "  \"outputs\" : [\n"
                "    {\n"
				"      \"output\" : n,                   (numeric, hush) the index of the output within the vShieldedOutput\n"
                "      \"address\" : \"qraxaddress\",     (string) The QRAX address involved in the transaction\n"
				"      \"outgoing\" : true|false         (boolean, hush) True if the output is not for an address in the wallet\n"
                "      \"value\" : x.xxx                 (numeric) The amount in " + CURRENCY_UNIT + "\n"
                "      \"valueSat\" : xxxx               (numeric) The amount in satoshis\n"
...
static SaplingOperation CreateShieldedTransaction(const JSONRPCRequest& request)
{
    ...
    // Check network status
    if (!Params().GetConsensus().NetworkUpgradeActive(nextBlockHeight, Consensus::ACTIVATE_HUSH) ||
...
UniValue shieldsendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "      \"amount\":amount    (numeric, required) The numeric amount in " + "QRAX" + " is the value\n"
                ...
                + HelpExampleCli("shieldsendmany",
								 "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" '[{\"address\": \"ps1ra969yfhvhp73rw5ak2xvtcm9fkuqsnmad7qln79mphhdrst3lwu9vvv03yuyqlh42p42st47qd\" ,\"amount\": 5.0}]'")
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"qraxaddress\",     (string) The qrax address\n"
            "      amount,                 (numeric) The amount in QRAX\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signmessage \"qraxaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"qraxaddress\"  (string, required) The qrax address to use for the private key.\n"
			...
			HelpExampleCli("signmessage", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" \"my message\"") +
            "\nVerify the signature\n" +
			HelpExampleCli("verifymessage", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" \"signature\" \"my message\"") +
            "\nAs json rpc\n" +
			HelpExampleRpc("signmessage", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\", \"my message\""));
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"qraxaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given qraxaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"qraxaddress\"  (string, required) The qrax address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in QRAX received at this address.\n"

            "\nExamples:\n"
            "\nThe amount from transactions with at least 1 confirmation\n" +
			HelpExampleCli("getreceivedbyaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\"") +
            "\nThe amount including unconfirmed transactions, zero confirmations\n" +
			HelpExampleCli("getreceivedbyaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" 0") +
            "\nThe amount with at least 6 confirmation, very safe\n" +
			HelpExampleCli("getreceivedbyaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" 6") +
            "\nAs a json rpc call\n" +
			HelpExampleRpc("getreceivedbyaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\", 6"));
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in QRAX received for this label.\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in QRAX received for this wallet in P2CS contracts.\n"
...
static UniValue legacy_sendmany(const UniValue& sendTo, int nMinDepth, std::string comment, bool fIncludeDelegated)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid QRAX address: ")+name_);
...
UniValue sendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shield QRAX addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The qrax address (either transparent or shield) is the key,\n"
            "                                     the numeric amount in QRAX is the value\n"
            "      ,...\n"
            "    }\n"
			...
			HelpExampleCli("sendmany", "\"\" \"{\\\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\\\":0.01,\\\"QfCo88MgvKy6BqB1SRPGnnAfb2va3XetbJ\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n" +
			HelpExampleCli("sendmany", "\"\" \"{\\\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\\\":0.01,\\\"QfCo88MgvKy6BqB1SRPGnnAfb2va3XetbJ\\\":0.02}\" 6 \"testing\"") +
            ...
            "\nAs a json rpc call\n" +
			HelpExampleRpc("sendmany", "\"\", \"{\\\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\\\":0.01,\\\"QfCo88MgvKy6BqB1SRPGnnAfb2va3XetbJ\\\":0.02}\", 6, \"testing\"")
        );
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a QRAX address or hex-encoded public key.\n"
            "If 'label' is specified, assign address to that label.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of qrax addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) qrax address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"label\"      (string, optional) A label to assign the addresses to.\n"

            "\nResult:\n"
            "\"qraxaddress\"  (string) A qrax address associated with the keys.\n"

            "\nExamples:\n"
            "\nAdd a multisig address from 2 addresses\n" +
			HelpExampleCli("addmultisigaddress", "2 \"[\\\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\\\",\\\"QfCo88MgvKy6BqB1SRPGnnAfb2va3XetbJ\\\"]\"") +
            "\nAs json rpc call\n" +
			HelpExampleRpc("addmultisigaddress", "2, \"[\\\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\\\",\\\"QfCo88MgvKy6BqB1SRPGnnAfb2va3XetbJ\\\"]\""));
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in QRAX received by the address\n"
			...
			HelpExampleRpc("listreceivedbyaddress", "6, true, true, \"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\""));
...
void ListTransactions(const CWalletTx& wtx, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if (!listSent.empty() || nFee != 0) {
        for (const COutputEntry& s : listSent) {
            ...
            //entry.pushKV("fee", !(wtx.IsCoinBase() || wtx.IsCoinStake()) ? ValueFromAmount(-nFee) : 0);
            if (fLong)
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"qraxaddress\",    (string) The qrax address of the transaction.\n"
            "    \"category\":\"category\",      (string) The transaction category (send|receive|orphan|immature|generate).\n"
            "    \"amount\": x.xxx,          (numeric) The amount in QRAX. This is negative for the 'send' category, and positive\n"
            "                                         for the 'receive' category,\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in QRAX. This is negative and only available for the \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"address\":\"qraxaddress\",    (string) The qrax address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in QRAX. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in QRAX. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in QRAX\n"
            "  \"confirmations\" : n,     (numeric) The number of confirmations\n"
            "  \"bcconfirmations\" : n,             (numeric) DEPRECATED: Will be removed in a future release\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in QRAX\n"
            "      \"label\" : \"label\",              (string) A comment for the address/transaction, if any\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending QRAXs\n"
...
UniValue walletlock(const JSONRPCRequest& request)
{
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 0))
        throw std::runtime_error(
			...
			HelpExampleCli("sendtoaddress", "\"QfmdnoFr17gBfLuLju4k16gPGHTXfexZnc\" 1.0") +
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending QRAXs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"qraxaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; qrax server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "1. minconf          (numeric, optional, default=1) The minimum confirmations to filter\n"
                "2. maxconf          (numeric, optional, default=9999999) The maximum confirmations to filter\n"
                "3. \"addresses\"    (string) A json array of qrax addresses to filter\n"
                "    [\n"
                "      \"address\"   (string) qrax address\n"
                "      ,...\n"
                "    ]\n"
                ...
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"address\" : \"address\",  (string) the qrax address\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid QRAX address: ") + input.get_str());
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending QRAXs.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in QRAX/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total QRAX balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the QRAX balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the QRAX balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in QRAX\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in QRAX\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in QRAX\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in QRAX\n"
            "  \"multimining_wallet_balance\": xxxxxx,    (numeric) the total multimining balance of the wallet in QRAX\n"
            "  \"multimining_structure_balance\": xxxxxx, (numeric) the total multimining balance of the structure in QRAX\n"
    ...
    obj.pushKV("asset_owner_balance",    ValueFromAmount(pwalletMain->GetMultiMiningBalance()));
    obj.pushKV("asset_structure_balance",    ValueFromAmount(pwalletMain->GetMultiMiningStructureBalance()));
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in qrax.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in qrax.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) QRAX value of the stakeable coins (minus reserve balance, if any)\n"
...
UniValue setstakesplitthreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 QRAX each)."
            + HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in QRAX).\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same QRAX address\n"
