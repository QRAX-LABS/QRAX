                                           ...
                                           bool selectTransparentCoins)
{
    ...
    CValidationState state;
    BOOST_ASSERT_MSG(
			CheckTransaction(operation.getFinalTx(), state, false, true),
...
BOOST_AUTO_TEST_CASE(test_in_block_and_mempool_notes_double_spend)
{
    ...
    UpdateNetworkUpgradeParameters(Consensus::ACTIVATE_HUSH, SAPLING_ACTIVATION_HEIGHT);
    setupWallet();
    ...
    // Now that we have the chain, let's shield 100 QRAXs
    // single recipient
    ...
    BOOST_CHECK_EQUAL(pwalletMain->GetAvailableShieldedBalance(), CAmount(100 * COIN)); // 100 shield QRAXs
    BOOST_CHECK_EQUAL(pwalletMain->GetUnconfirmedShieldedBalance(), CAmount(0)); // 0 shield QRAXs
