...
/*
BOOST_AUTO_TEST_CASE(test_fee_calculate)
{
    CAmount nValue;
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        CAmount fee = pwalletMain->GetPercentFee(0.0001 * COIN);
...
BOOST_AUTO_TEST_CASE(multimining_db_tests)
{
    /*CWallet &wallet = *pwalletMain;
    LOCK2(cs_main, wallet.cs_wallet);
    wallet.SetMinVersion(FEATURE_PRE_SPLIT_KEYPOOL);
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        CTxDestination receivingAddr;
        BOOST_ASSERT(wallet.getNewAddress(receivingAddr, "receiving_address").result);
        BOOST_CHECK(IsValidDestination(receivingAddr));
        CKeyID keyID = *boost::get<CKeyID>(&receivingAddr);
        multiMiningManager.GenerateWalletIdent(&keyID);
    }*/
