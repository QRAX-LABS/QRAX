...
/**
 * Fees smaller than this (in uqrax) are considered zero fee (for transaction creation)
...
bool CWallet::ActivateSaplingWallet(bool memOnly)
{
    if (m_sspk_man->SetupGeneration(m_spk_man->GetHDChain().GetID(), true, memOnly)) {
		LogPrintf("%s : hush spkm setup completed\n", __func__);
        // Just to be triple sure, if the version isn't updated, set it.
        if (!SetMinVersion(WalletFeature::FEATURE_SAPLING)) {
			LogPrintf("%s : ERROR: wallet cannot upgrade to hush features. Try to upgrade using the 'upgradewallet' RPC command\n", __func__);
            return false;
        }
        return true;
    }
    ...
    for (std::pair<const uint256, CWalletTx>& wtxItem : mapWallet) {
        ...
        if (!wtx.mapSaplingNoteData.empty()) {
            ...
            if (!wtx.tx->isSaplingVersion()) {
				LogPrintf("SetBestChain(): ERROR, Invalid tx version found with hush data\n");
                walletdb.TxnAbort();
...
std::set<uint256> CWallet::GetConflicts(const uint256& txid) const
{
    ...
    for (const CTxIn& txin : wtx.tx->vin) {
        if (mapTxSpends.count(txin.prevout) <= 1 || wtx.ContainsAssets())
...
void CWallet::AddToSpends(const uint256& wtxid)
{
    ...
    if (thisTx.IsCoinBase()|| thisTx.ContainsAssets()) // Coinbases don't spend anything!
        return;
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose)
{
	...
	NotifyTransactionChanged(hash, fInsertedNew ? CT_NEW : CT_UPDATED, 1);
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const CWalletTx::Confirmation& confirm, bool fUpdate)
{
    const CTransaction& tx = *ptx;
    {
        ...
        if (!confirm.hashBlock.IsNull() && !tx.ContainsAssets() && !tx.IsCoinBase()) {
...
bool CWallet::AbandonTransaction(const uint256& hashTx)
{
    ...
    while (!todo.empty()) {
        ...
        if (currentconfirm == 0 && !wtx.isAbandoned()) {
			...
			NotifyTransactionChanged(wtx.GetHash(), CT_UPDATED, 0);
...
void CWallet::BlockDisconnected(const std::shared_ptr<const CBlock>& pblock, const uint256& blockHash, int nBlockHeight, int64_t blockTime)
{
    ...
    if (Params().GetConsensus().NetworkUpgradeActive(nBlockHeight, Consensus::ACTIVATE_HUSH)) {
...

    {
        ...
        while (pindex) {
...

            {
                ...
                if (pindex->pprev) {
                    if (Params().GetConsensus().NetworkUpgradeActive(pindex->pprev->nHeight, Consensus::ACTIVATE_HUSH)) {
                        SaplingMerkleTree saplingTree;
...
void CWallet::ReacceptWalletTransactions(bool fFirstLoad)
{
	std::map<int64_t, CWalletTx*> mapSorted;
	{
		...
		for (std::pair<const uint256, CWalletTx>& item: mapWallet) {
			...
			if (!wtx.IsCoinBase() && !wtx.IsCoinStake() && !wtx.isAbandoned() && !wtx.ContainsAssets()) {
	...
	const int count = mapSorted.size();
	ShowProgress(_("Reaccepting wallet transactions..."), 0);
    ...
    for (std::pair<const int64_t, CWalletTx*>& item: mapSorted) {
		...
		bool fSuccess = wtx.AcceptToMemoryPool(state, false, true, false, fFirstLoad);
		...
		counter++;
		ShowProgress(_("Reaccepting wallet transactions..."), (counter * 100 / count));
    }

	ShowProgress(_("Reaccepting wallet transactions..."), 100);
}
...
{
    if (!connman || IsCoinBase() || IsCoinStake() || ContainsAssets()) {
...
std::vector<CAssetNodeHistoryValue> CWallet::GetAssetHistoryList(const int &nHeight)
{
    return multiMiningDB->LoadAssetRewardHistory(nHeight);
}

CAmount CWallet::GetAssetPercentDowngradeProfit()
{
    return multiMiningManager.GetOwnPercentDowngradeProfit();
}

CAmount CWallet::GetMultiMiningBalance()
{
    return multiMiningManager.GetOwnMultiMiningBalance();
}

CAmount CWallet::GetMultiMiningStructureBalance()
{
    return multiMiningManager.GetOwnStructureBalance();
}

double CWallet::GetMultiMiningPercent()
{
    return multiMiningManager.GetOwnPercentProgram();
}

CAssetNode *CWallet::GetMultiMiningStructureTree()
{
    return multiMiningManager.GetMultiMiningStructure();
}

CAssetNode *CWallet::GetAssetNodeInformation(const CKeyID &key)
{
    return multiMiningManager.GetMultiMiningStructure(key);
}

int CWallet::GetAssetNodeTime() const
{
	int nHeight = m_last_block_processed_height;
    if (!Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::ACTIVATE_POS)) return 0;

    return multiMiningManager.GetMultiminingNodeTime();
}
...
CAmount CWallet::GetAvailableBalanceAtHeight(isminefilter &filter, bool useCache, int nHeight) const
{
	return loopTxsBalance([filter, useCache, nHeight](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal){
		bool fConflicted;
		int depth;
		if (pcoin.IsTrusted(depth, fConflicted) && pcoin.m_confirm.block_height <= nHeight) {
			nTotal +=  (pcoin.GetCredit(filter) - pcoin.GetDebit(filter));
		}
	});
}
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != MN_COLL_AMT) {
        strError = "Invalid collateral tx value, must be 10,000 QRAX";
    ...
    for (const auto& it : mapWallet) {
        ...
        for (unsigned int index = 0; index < pcoin->tx->vout.size(); index++) {
            ...
            auto res = CheckOutputAvailability(
					...
					fIncludeColdStaking, // fIncludeColdStaking
    ...
    bool* fBurnAmountTransaction)
{
...

    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
			...
			std::string genesisKey = consensus.strGenesisPubKeyHex;
            std::vector<unsigned char> data(ParseHex(genesisKey));
            ...
            while (true) {
                ...
                // Fill outputs
                for (const CRecipient& rec : vecSend) {
                    bool toGenesisAddress = false;
                    CAmount nTxAmount = rec.nAmount;
                    uint160 curhash;
                    if (rec.scriptPubKey.IsPayToScriptHash()) {
                        std::vector<unsigned char> hashBytes(rec.scriptPubKey.begin()+2, rec.scriptPubKey.begin()+22);
                        curhash = uint160(hashBytes);
                    } else if (rec.scriptPubKey.IsPayToPublicKeyHash()) {
                        std::vector<unsigned char> hashBytes(rec.scriptPubKey.begin()+3, rec.scriptPubKey.begin()+23);
                        curhash = uint160(hashBytes);
                    } else if (rec.scriptPubKey.IsPayToPublicKey()) {
                        curhash = uint160(Hash160(rec.scriptPubKey.begin()+1, rec.scriptPubKey.end()-1));
                    }
                    if (fBurnAmountTransaction && curhash == pubKey.GetID()) {
                        nTxAmount = 0;
                        *fBurnAmountTransaction = true;
                        toGenesisAddress = true;
                    }

                    CTxOut txout(nTxAmount, rec.scriptPubKey);
                    if (toGenesisAddress == false && IsDust(txout, ::minRelayTxFee)) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-qrax-address
                ...
                if (!fMultiWalletsNode) {
                    txNew.walletSenderId = GetWalletIdentHash();
                }

                bool txWalletSenderIsSet = false;
                // Fill vin
                for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
                    ...
                    if (fMultiWalletsNode) {
                        if (txWalletSenderIsSet) continue;
                        uint160 curhash;
                        if (coin.first->tx->vout[coin.second].scriptPubKey.IsPayToScriptHash()) {
                            std::vector<unsigned char> hashBytes(coin.first->tx->vout[coin.second].scriptPubKey.begin()+2, coin.first->tx->vout[coin.second].scriptPubKey.begin()+22);
                            curhash = uint160(hashBytes);
                        } else if (coin.first->tx->vout[coin.second].scriptPubKey.IsPayToPublicKeyHash()) {
                            std::vector<unsigned char> hashBytes(coin.first->tx->vout[coin.second].scriptPubKey.begin()+3, coin.first->tx->vout[coin.second].scriptPubKey.begin()+23);
                            curhash = uint160(hashBytes);
                        } else if (coin.first->tx->vout[coin.second].scriptPubKey.IsPayToPublicKey()) {
                            curhash = uint160(Hash160(coin.first->tx->vout[coin.second].scriptPubKey.begin()+1, coin.first->tx->vout[coin.second].scriptPubKey.end()-1));
                        }
                        txNew.walletSenderId = CKeyID(curhash);
                        txWalletSenderIsSet = true;
                    }
				...
				CAmount nFeeNeeded = ::minRelayTxFee.GetPercentFee(nValue, isNewFee); //std::max(nFeePay, GetMinimumFee(nBytes, nTxConfirmTarget, mempool));
                ...
                if (coinControl && coinControl->fOverrideFeeRate)
                    nFeeNeeded = coinControl->nFeeRate.GetPercentFee(nValue); //(nBytes);
                */

                // If we made it here and we aren't even able to meet the relay fee on the next pass, give up
                // because we must be at the maximum allowed fee.
                /*if (nFeeNeeded < ::minRelayTxFee.GetPercentFee(nValue)) {
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CTransactionRef& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, CAmount nFeePay, bool fIncludeDelegated)
{
    ...
    int nChangePosInOut = -1;
    bool P2CSVoided = false;
    bool toGenesisPay = false;
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePosInOut, strFailReason, coinControl, coin_type, true, nFeePay, fIncludeDelegated, &P2CSVoided, &toGenesisPay);
}

bool CWallet::CreateAssetRewardTransactions(CMutableTransaction& txNew)
{
	const int nHeight = GetLastBlockHeightLockWallet();

    txNew.vin.clear();
    txNew.vout.clear();

    txNew.walletSenderId = multiMiningManager.getGenesisKeyId();
    txNew.vin.emplace_back();
	txNew.vin[0].scriptSig = CScript() << OP_ASSETSSPEND << nHeight + 1 << OP_0;

    std::vector<CAssetNode*> nodes = multiMiningManager.GetAvailableNodesToRewardMultimining();
    bool isFilled = false;

    if (nodes.size() > 0) {

        for (CAssetNode* curNode : nodes)
        {
            double delegatorProfit = 0;
            double nodeProfit = 0;
            int index1 = 0;
            CAddressOwnerIdentificator owner;
            if (!GetAddressIdentification(curNode->getKeyId(), CChainParams::PUBKEY_ADDRESS, owner)) continue;
            if (curNode->currentProfit > 0) {
                txNew.vout.emplace_back(CTxOut(curNode->currentProfit, GetScriptForMultiminingDestination(curNode->getKeyId())));
            } else {
                txNew.vout.emplace_back(CTxOut(0, GetScriptForMultiminingDestination(curNode->getKeyId())));
            }

            index1 = txNew.vout.size() - 1;

            for (CAssetDelegation* delegate : curNode->delegations) {
				if (delegate->getProfit() > 0 && delegate->isNeedPaid() && nHeight >= delegate->getHeight()) {
                    delegatorProfit = delegate->getProfit() * 90 / 100;
                    nodeProfit += (delegate->getProfit() - delegatorProfit);
                    txNew.vout.emplace_back(CTxOut(delegatorProfit, GetScriptForAssetsDposDestination(delegate->getOwnerKey(), delegate->getStakerKey())));
                }
            }
            txNew.vout[index1].nValue += nodeProfit;
			if (txNew.vout[index1].nValue <= 0) {
				txNew.vout.pop_back();
				continue;
			}
        }
		...
		std::vector<CStakeableOutput>* availableCoins)
{
    ...
    bool onlyP2PK = !consensus.NetworkUpgradeActive(pindexPrev->nHeight + 1, Consensus::ACTIVATE_HUSH);
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
		...
		CNewStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);

        // New block came in, move on
		if (GetLastBlockHeightLockWallet() != pindexPrev->nHeight) return false;
        ...
        //LogPrintf("%s: out %s\n", __func__, out.tx->GetHash().ToString().c_str());
		...
		scriptPubKeyKernel = vout.begin()->scriptPubKey;
	...
	if (!fMultiWalletsNode) {
		// Set ident hash in tx
		txNew.walletSenderId = pwalletMain->GetWalletIdentHash();
	} else {
		std::vector<uint160> key = GetHashBytesFromScriptPubKey(scriptPubKeyKernel);
		if (key.size() > 0) {
			txNew.walletSenderId = CKeyID(key[0]);
		} else
			txNew.walletSenderId = multiMiningManager.getGenesisKeyId();
	}
...

    {
        ...
        LogPrintf("%s:\n%s", __func__, wtxNew.tx->ToString());
        {
			...
			for (const CTxIn& txin : wtxNew.tx->vin) {
				...
				NotifyTransactionChanged(txin.prevout.hash, CT_UPDATED, 1);
        ...
        CValidationState state;
        if (!wtx.AcceptToMemoryPool(state, true, false, false)) {
            res.state = state;
...
CAmount CWallet::GetFeeByAmount(CAmount value) const
{
	const CChainParams& params = Params();
	const Consensus::Params& consensus = params.GetConsensus();
	int chainHeight = chainActive.Height();
	bool isNewFee = consensus.NetworkUpgradeActive(chainHeight, Consensus::ACTIVATE_NEW_FEE_RULES);
	return ::minRelayTxFee.GetPercentFee(value, isNewFee);
}
...
bool CReserveKey::GetReservedKey(CPubKey& pubkey, bool _internal)
{
    ...
    if (nIndex == -1) {
        ...
        changeType = internal ? HDChain::ChangeType::INTERNAL : HDChain::ChangeType::EXTERNAL;
        CKeyPool keypool;
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in QRAX) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of QRAX Core"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart QRAX Core to complete"));
            return nullptr;
        } else {
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
		...
		if (prev_version <= FEATURE_PRE_QRAX && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
			...
			walletInstance->SetMaxVersion(FEATURE_PRE_QRAX);
        }
...
CKeyID CWallet::GetWalletIdentHash()
{
    return multiMiningManager.getKeyId();
}

CKeyID CWallet::GetGenesisMultiMiningHash()
{
    return multiMiningManager.getGenesisKeyId();
}

void CWallet::SetAssetWalletIdent(const CKeyID &keyId)
{

}

void CWallet::SetAssetParentWalletIdent(const CKeyID &parentId)
{
...
int CWalletTx::GetDepthInMainChain() const
{
	...
	return (pwallet->GetLastBlockHeightLockWallet() - m_confirm.block_height + 1) * (isConflicted() ? -1 : 1);
}
...
bool CWalletTx::AcceptToMemoryPool(CValidationState& state, bool fLimitFree, bool fRejectInsaneFee, bool ignoreFees, bool bLoadingWallet)
{
	{
...
void CWallet::SetNull()
{
    ...
    // User-defined fee QRAX/kb
    fUseCustomFee = false;
    nCustomFee = CWallet::minTxFee.GetPercentFee(0);
...
{
    {
		...
		if (!IsFinalTx(tx, pwallet->GetLastBlockHeightLockWallet()))
            return false;
    }
