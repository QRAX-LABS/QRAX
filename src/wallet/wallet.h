...
// masternode pay amount
static const CAmount MASTERNODE_PAYMENT_AMOUNT = 50000 * COIN;
...
static const CAmount nHighTransactionMaxFeeWarning = 10 * nHighTransactionFeeWarning;
...
enum WalletFeature {
	...
	FEATURE_PRE_QRAX = 61000, // inherited version..
...
public:
    ...
    mutable bool fBurnAmountTransaction;
    ...
    CAmount GetTxFeeByAmount() const {return tx->GetTxFeeByAmount(); };
    ...
    bool ContainsAssets() const {return tx->ContainsAssets(); };

    CKeyID GetWalletId() const {return tx->walletSenderId; };

    /** Pass this transaction to the mempool. Fails if absolute fee exceeds absurd fee. */
	bool AcceptToMemoryPool(CValidationState& state, bool fLimitFree = true, bool fRejectInsaneFee = true, bool ignoreFees = false, bool bLoadingWallet = false);
...
public:
    ...
    /// Get 10000 QRAX output and keys which can be used for the Masternode
    ...
    bool CreateTransaction(const std::vector<CRecipient>& vecSend,
        ...
        bool* fBurnAmountTransaction = nullptr);
     ...
     * @note called with lock cs_wallet held.
     */
	boost::signals2::signal<void(const uint256& hashTx, ChangeType status, int showTransaction)> NotifyTransactionChanged;
    ...
    void SetAssetWalletIdent(const CKeyID &keyId);
    void SetAssetParentWalletIdent(const CKeyID &parenId);

    CKeyID GetWalletIdentHash();
    CKeyID GetGenesisMultiMiningHash();
    CAmount GetMultiMiningBalance();
    CAmount GetMultiMiningStructureBalance();
    double GetMultiMiningPercent();
	bool CreateAssetRewardTransactions(CMutableTransaction &txNew);
    CAssetNode *GetMultiMiningStructureTree();
    int GetAssetNodeTime() const;
