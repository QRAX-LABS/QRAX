...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        } else if (strType == "walletIdentification") {
            CKeyID walletIdent;
            ssValue >> walletIdent;
            if (!walletIdent.IsNull()) {
				multiMiningManager.setKeyId(walletIdent);
            }
        } else if (strType == "parentIndetification") {
            CKeyID parentIdent;
            ssValue >> parentIdent;
            if (!parentIdent.IsNull()) {
                multiMiningManager.setParentId(parentIdent);
            }
        }
    } catch (...) {
        return false;
...
bool CWalletDB::ReadAssetWalletIdentKey(CKeyID &keyId)
{
    CKeyID key;
    if (batch.Read(std::string("walletIdentification"), key))
    {
        keyId = key;
        return true;
    }
...
bool CWalletDB::WriteAssetWalletIdentKey(const CKeyID &keyId)
{
    IncrementUpdateCounter();
    return batch.Write(std::string("walletIdentification"), keyId);
}

bool CWalletDB::WriteAssetParentWalletIdentKey(const CKeyID &hash)
{  
    IncrementUpdateCounter();
    return batch.Write(std::string("parentIndetification"), hash);
}

bool CWalletDB::EraseInvalidParentWalletIdent()
{
	return batch.Erase(std::string("walletIdentification"));
}

bool CWalletDB::ReadAssetParentWalletIdentKey(CKeyID &key)
{
    return batch.Read(std::string("parentIndetification"), key);
}
