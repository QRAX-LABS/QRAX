...
class ChainstateWriteCrashTest(QraxTestFramework):
	...
	self.chain_params = ['-nuparams=v5_shield:90000', '-nuparams=QRAX_v4.0:90000',
	                     '-nuparams=QRAX_v3.4:90000',
    ...
    def restart_node(self, node_index, expected_tip):
		...
		# If qraxd exits, then try again.  wait_for_node_exit()
		# should raise an exception if qraxd doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

	# If we got here, qraxd isn't coming back up on restart.  Could be a
	# bug in qraxd, or we've gotten unlucky with our dbcrash ratio --
