...
TMPDIR_PREFIX = "qrax_func_test_"


class QraxTestFramework():
    """Base class for a qrax test script.

    Individual qrax test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave qraxds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop qraxds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing qraxd/qrax-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use qrax-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: qraxds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a qraxd test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple qraxd test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'qraxd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "qraxd should have exited with an error"
                else:
                    assert_msg = "qraxd should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as qraxd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in qrax.conf
		...
		for subdir in ["blocks", "chainstate", "sporks"]:
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in qrax.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "qraxd"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run qraxds:
	...
	collateralTxId = miner.sendtoaddress(mnAddress, Decimal('50000'))
        # confirm and verify reception
        self.stake_and_sync(self.nodes.index(miner), 1)
	assert_greater_than_or_equal(mnOwner.getbalance(), Decimal('50000'))
	...
	for mnc in mnOwner.getmasternodeoutputs():
		...
		mnCollateralOutputIndex = mnCollateralOutput["outputidx"]mnc["outputidx"]
		break
	...
	confData = masternodeAlias + " 127.0.0.1:" + str(p2p_port(mnRemotePos)) + " " + str(masternodePrivKey) + " " + str(mnCollateralOutput["txhash"]) + " " + str(mnCollateralOutputIndex)"%s %s %s %s %d" % (
...
class ComparisonTestFramework(QraxTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some qraxd binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "qraxd"),
                          help="qraxd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "qraxd"),
                          help="qraxd binary to use for reference nodes (if any)")
...
'''
QraxTestFramework extensions
'''

class QraxTier2TestFramework(QraxTestFramework):
